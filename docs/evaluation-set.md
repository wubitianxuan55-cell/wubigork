...
| E02 | a52c6ce（不可达） | 角色注入 prompt 夹带剧照 base64，摘要抽取"我"等代词 | 剔除 base64、过滤代词 | 已修复 + 回归（types.PromptView 剥离剧照；角色/大纲/分析注入统一收口；character_test.go 断言） |
| E03 | da2d5b8（不可达） | 章节创作回退全局模型而非功能绑定模型 | 使用功能级模型绑定 | 已转回归测试（章节/分支/Cmd+K/风格走 routeModel；5 个 agent 未绑定回退按引擎解析默认模型） |

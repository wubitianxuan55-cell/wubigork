...
### 14.7 底稿面板内编辑（v4.266.0）

v4.263 下刀候选清账（「用户直接改底稿的冲突合并留待下刀」）。此前设定集/大纲是
只读视图，写路径只在工具侧（AI 写作）。

**冲突合并设计（本刀的设计决定）= 回合互斥 + 锁内基线比对确认，不做字段级 merge**：

1. AI 是底稿唯一的机器写者，且只在回合内写（工具循环）。面板编辑在 `sending`
   中锁定（入口禁用 + 草稿保存禁用）→ 常规路径结构上无并发写。
2. 残余并发窗口（多开壳实例/他端写入）：保存走 `SinNotesSave`，在 `sinNotesMu`
   锁内做**基线比对**——入参带用户开始编辑时的 doc 快照 JSON，与当前文件不符即
   拒绝（错误固定前缀「底稿冲突：」），前端弹「覆盖确认」，确认后 `force=true`
   重试。静默覆盖不可能发生。
3. 限长与工具侧完全同口径：单条 2000 rune 截断、大纲 4000 rune 截断、条数 200
   上限（超限拒绝不静默丢）——用户编辑与 AI 写作遵守同一套规则。

**编辑 UX**：大纲页签「编辑/写大纲」→ textarea（rune 计数 x/4000）；设定页签
「编辑/记便签」→ 逐条 textarea + 删除 + 添加一条（≤200 条提示）；保存/取消；
sending 中保存禁用并给原因。空底稿也给入口（用户可以先于 AI 手动起草）。

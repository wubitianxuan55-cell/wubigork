...
> 状态：**本设计（S2.1）已随 v3.9.0 落地收官**（`8cd1bec` 两视图+空间切换+双首页；同版
> S2.2 `10c9b8a`、S2.3 `c6a699b`、S2.3b `295c20c`、页面迁入 P1 `bb76d48`，见
> `releases/v3.9.0.md`）。全文属「10 板块→双空间」旧壳时代历史设计，现权威 =
> `docs/gaea-nextgen-roadmap-2026.md` 与现行代码；因前端/后端约 20 处代码注释仍引用
> 本文件为设计出处（docs/README.md「活跃设计」在列），留 docs/ 原位未移入 archive/。

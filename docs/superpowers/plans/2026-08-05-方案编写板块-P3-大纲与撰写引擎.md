...
**Goal:** 实现 15 万字级技术方案的分章节批量生成：目录策略 + 字数预算分解 + 项目事实基线 + 章节上下文升级 + 批量生成队列（断点续写/合并装配）+ 图表预设升级。
...
// DefaultTotalWords 默认总字数（15 万字级技术方案）
const DefaultTotalWords = 150000
```
...
1. 新增状态：`const [strategy, setStrategy] = useState('reference'); const [totalWords, setTotalWords] = useState(150000); const [oiOpen, setOiOpen] = useState(false); const [oiMd, setOiMd] = useState('')`
2. 工具条增加：
...
<InputNumber value={totalWords} onChange={v => setTotalWords(v || 150000)} min={10000} step={10000} style={{ width: 130 }} />
...
1. 新建方案 → 选择目录策略与总字数 → 生成大纲，章节显示字数目标。

...
- **定位：可选编排入口，不是必经路径。** 各模块保持独立直达（方案/轻语/绘梦等页面直接调各自模型，不经主脑转达）；主脑只在两类情况介入：
  1. 用户不知道找哪个模块（"帮我把这个标书写了" → 主脑指路/代发）；
  2. 跨模块编排（"用轻语记住的甲方偏好写标书" → 取右脑资料、派发左脑方案）。
- 流程：意图识别（规则 + LLM）→ 取两脑资料（`BrainStore`）→ 派发模块（`ModuleRegistry`）→ 汇总回复。
- 1.x 办公 agent 能力保留：GAEA 从"办公专属"升级为"全模块协调者"。
- P3 只做后端能力（`MainBrainChat` 绑定），**不做前端主脑页**（入口形态留待 3.0 单窗口编排时决定）。

### 7.3 验收

- 一句话任务派发到正确模块（如"帮我把这个标书写了"→ 方案模块）。
- 派发时自动注入相关脑资料（右脑甲方信息 → 左脑标书上下文）。
- 各模块直达路径不受影响（不经主脑的原有页面流程保持不变）。

## 8. 前端范围

- 保留 1.x 全部页面与 IA。
- 新增：模型中心"每功能绑定"管理页（强化 1.x FeatureModelBar）；记忆中枢三脑视图（主/左/右 + 跨脑关联展示）。
- **不做**：主脑对话页（主脑是可选编排入口，P3 仅后端能力；UI 形态留待 3.0）。

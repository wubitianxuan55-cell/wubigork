...
> 日期：2026-08-15 ｜ 状态：v1.0 定稿 ｜ 范围：架构主线改造
...
- **顺序**：Step 0 修债 → Step 1 事件日志 → Step 2 板块 Manifest → Step 3 Provider Seam → 发布 3.0.0。编程板块当时不在 3.0.0 范围内（2026-08-16 已恢复落地）；Manifest 验证试点由知识库与 chat 承担。
...
## 9. 编程板块（DeepSeek Harness Web 进程管理）

> **2026-08-16 落地：** 编程板块以「DeepSeek Harness Web 进程管理」形态实现——
> 后端 `internal/app/programming_web.go` 提供 GetProgrammingWebStatus / StartProgrammingWeb /

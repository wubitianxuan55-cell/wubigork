...
| # | 面 | 现状实测 | 目标态（6 周） | 长期态 |
|---|---|---|---|---|
| 一 | 认知 | 13 一级板块（work5/play4/shared3/indep1），办公核心被稀释 | 工位一级 ≤6：schedule 并入办公文档面、乐园折叠 | 板块数只减不增；新域一律先问「能否是文档」 |
...
2. **乐园折叠**：chat/novel/imagegen/characterlib 收进空间切换器（manifests `space:'play'`
   已就绪），home 不再陈列全板块。
3. **home 重定位「今日工作台」**：最近工程/文档/待办/模板。
...
| **P2 形态** | 2 版 | 白名单解耦刀0→schedule 合并刀1/刀2→乐园折叠→home 重定位→三路可达走查（壳内真机） | 工位一级 ≤6；等价快照表全绿 |
...
```
工位一级板块数 ｜ entry chunk(gz KB) ｜ 冷启动可交互(ms) ｜ 常驻内存(MB) ｜ exe(MB) ｜ >50KB 源文件数
```

...
- [x] **Step 1: 修改 package.json，新增 test 脚本**
...
- [x] **Step 4: 修改 vite.config.ts 接入 test 配置**
...
- **Vitest 设施**：`vitest@4 / jsdom@30 / @testing-library/react@16` 安装完成；`test` 脚本与 `vite.config.ts` 的 `test` 配置已接入；`src/test/setup.ts` 自动 cleanup。
- **CharacterCard 组件**：档案眉（NO.xxx 编号 / 类型标签 / 可聊天标记）、4:3 立绘横幅（图片或首字占位 + 悬停光泽）、正文（名称 / 发丝线元数据 / 弧线引言 / 最多 3 个 # 标签）、底部（五维雷达 + 悬停浮现操作区）、当前人格角标、已加入标记；`prefers-reduced-motion` 全部降级。9 个 Vitest 用例 PASS。
- **列表页接入**：网格改用 `CharacterCard`；头部工具栏改为发丝线分隔；新增“当前人格”状态（localStorage 初始化 + `gaea-persona-changed` 事件同步，避免命名遮蔽模块级 `setCurrentPersona` 持久化函数）；旧内联卡片代码与不再使用的导入已清理。
- **验证**：`npm run test` 9/9 PASS；`npx tsc -b` 无错误；`npm run build` 成功；`scripts/ci.ps1`（go build/vet/test + 前端构建 + E 系列回归守卫）CI OK；`wails build` 成功产出 `build/bin/gaea.exe`。

与计划的两处小偏差：
1. 测试中雷达断言改为 `.ccard-foot > svg`（antd 图标本身也是 `<svg>`，原“无 svg”断言过宽）。
2. `makeCharacter` 返回类型用 `as unknown as LibraryCharacter` 转换（Wails 生成的 `Character` 类含必需方法，普通对象字面量不满足类型）。

待人工验证（未勾选）：桌面应用 UI 目检（Task 4 Step 2）与聊天联动回归（Task 4 Step 3）。

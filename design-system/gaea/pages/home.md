...
## v8 书斋重设计（2026-09-15 · 「驾驶舱台面」，技能库 ui-ux-pro-max 驱动）
**诊断**（Edge 无头目检 + analyze_image）：v7 杂志刊头大标题占 ~10% 视高信息价值极低；
左栏长卷（刊头→账页→目录）目录大部分在折叠线下；账页与目录两段同构行式列表纵向堆叠；
右栏五节等权无主次。
**方案**（零功能删除，纯重组；Swiss 数据密集 + 星枢令牌）：
- 刊头压缩：w-mast（大标题+三行 lede）→ w-deck 一条 kicker 行（chip + 单行 lede 截断 + rule + pill）；
  命令台升格为第一主角件（w-deck 玻璃抬升面：accent 渐变 + 顶部 1px 光线 + 气泡流/命令条/语音状态全收台内）。
- 账页 × 目录双列（w-columns：7fr/5fr，≤1180 单列塌缩账页在上）：同构列表并排消扫视疲劳；
  旗舰横带压缩（padding 20→11、水印 96→68px）适配窄列；w-index 双列改单列（密度对齐账页）。
- 右栏主从：晨报卡置顶（主动作卡），遥测/写作/会话/记忆四节退后（状态脉搏）。
- 响应式：≤1180 w-columns 单列 + deck-sub 隐藏；既有 ≤900/≤640 规则原样。
**契约不变**：ml-space-chip / desk-recent-docs / ml-space-* testid 全保留；i18n 零键增删

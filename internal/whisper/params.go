...
const (
	// B1: 长期心境 EWMA 收敛系数——每轮向即时情绪靠拢 1%（慢速，量纲 -100..100）。
	MoodAlpha = 0.01
	// B2: 心境低落判定阈值——Mood 的 (Aro+Aff)/2 均值低于此值视为低落，
	// 语音合成据此施加低落偏移（如 Speed*=0.95，由 voice_manager 接线使用）。
	MoodLowAroAffMean = -20.0
)

...
// SettleDesiresForKnowledgeTopic 知识整理后沉淀相关欲望
func SettleDesiresForKnowledgeTopic(stack DesireStack, knowledgeTopic string) DesireStack {
	topic := strings.TrimSpace(knowledgeTopic)
	if topic == "" {
		return stack
	}
	slots := make([]*Desire, len(stack.Slots))
	copy(slots, stack.Slots)
	for i, s := range slots {
		if s == nil || s.Status == "settled" {
			continue
		}
		if DesireTopicMatchesKnowledge(s.Topic, topic) {
			settled := *s
	...
	return DesireStack{Slots: slots}
}

// ─── 手动管理 ──────────────────────────────────────────────────

// DismissDesireFromStack 手动移除单条欲望
func DismissDesireFromStack(stack DesireStack, desireID string) DesireStack {
	id := strings.TrimSpace(desireID)
	if id == "" {
		return stack
	}
	slots := make([]*Desire, len(stack.Slots))
...
// ClearActiveDesires 清空所有 active 状态的欲望
func ClearActiveDesires(stack DesireStack) DesireStack {
	slots := make([]*Desire, len(stack.Slots))
	copy(slots, stack.Slots)
	for i, s := range slots {

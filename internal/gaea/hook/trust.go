...
// Trust gates project hooks. A project's .gaea/settings.json can run
...
// root. The trust flag lives in user-global state (~/.gaea/trust.json),
// NOT in the project file itself — an attacker controls the latter. Global
// hooks (~/.gaea/settings.json) are the user's own and always run.

// TrustFilename is the user-global trust store under ~/.gaea.
...
// TrustPath is ~/.gaea/trust.json (homeDir overrides ~).

...
// (.gaea/settings.json, only when the project is trusted) and a global
// (~/.gaea/settings.json) file. A hook's exit
...
const (
	SettingsDirname  = ".gaea"
	SettingsFilename = "settings.json"
)

// GlobalSettingsPath is ~/.gaea/settings.json (homeDir overrides ~).
func GlobalSettingsPath(homeDir string) string {
	return filepath.Join(home(homeDir), SettingsDirname, SettingsFilename)
}

// ProjectSettingsPath is <root>/.gaea/settings.json.

...
type Messages struct {
	...
	StepScaffold    string // step 1 desc — gaea setup
	StepSetKey      string // step 2 command label

	// `gaea init` — points to the in-session /init skill + setup
	InitHint       string
	StepSetKeyHint string // step 2 desc — env var hint
	StepChatDesc   string // gaea chat step desc
	StepRunDesc    string // gaea run step desc
	HelpFooter     string // dim footer linking to gaea help
	...
	WroteFileFmt          string // "Wrote %s" — used for gaea.toml and .env both
	...
	UsageRunHint      string // "usage: gaea run [--model NAME] <task>"

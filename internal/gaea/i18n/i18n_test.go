...
// $LC_ALL / $TIANXUAN_LANG. Unknown locales return "" so DetectLanguage falls
...
// TestDetectLanguagePriority verifies override beats env and that TIANXUAN_LANG
// beats LANG. With a clean env we fall back to English.
func TestDetectLanguagePriority(t *testing.T) {
	t.Setenv("TIANXUAN_LANG", "")
	t.Setenv("LC_ALL", "")
	...
	t.Setenv("TIANXUAN_LANG", "en")
	if got := DetectLanguage(""); got != "en" {
		t.Errorf("TIANXUAN_LANG=en overriding LANG=zh: got %q, want en", got)
	}

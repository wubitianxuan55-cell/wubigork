...
var English = Messages{
	...
	SetKeyHint:      "Set your API key, then run `gaea chat`.",
	ConfigLabel:     "config",
	ModelsLabel:     "models",
	...
	StepScaffold:    "scaffold gaea.toml",
	StepSetKey:      "set API key",

	InitHint:       "Project memory (AGENTS.md) is generated in-session: run `gaea chat`, then `/init` — the model analyzes the codebase and writes it. For configuration, use `gaea setup`.",
	StepSetKeyHint: "export DEEPSEEK_API_KEY=… or add to .env",
	StepChatDesc:   "interactive session",
	StepRunDesc:    "one-shot task",
	HelpFooter:     "gaea help · all commands",

	ChatTip:           "Context is kept across turns. Type 'exit' or Ctrl-D to quit.",
	TurnCancelled:     "cancelled — back to prompt",
	NoSessionToResume: "no saved session to resume — start a new one with `gaea chat`",
	...
	OutputStyleHint:   "set agent.output_style in gaea.toml to apply one (takes effect next session)",
	...
	SlashMCPNone:       "no MCP servers configured — add a [[plugins]] entry in gaea.toml",
	...
	CmdContext:      "open the context dashboard",
	...
	ListHooksNone:       "hooks: none active — configure in .gaea/settings.json (project, after trust) or ~/.gaea/settings.json (global)",
	ListMcpHeader:       "mcp servers",
	ListMcpNone:         "mcp: no servers connected — add one in gaea.toml ([[plugins]]) or a project .mcp.json",

	SelectProvidersLabel:  "Select providers to enable",
	EnterAPIKeysHeader:    "Enter API keys (Enter to skip and set later in .env):",
	MissingKeyIntro:       "gaea.toml is ready — just an API key away.",
	WroteFileFmt:          "Wrote %s",
	SetupComplete:         "Setup complete.",
	SetupCancelled:        "setup cancelled.",
	TryHintFmt:            "Try: %s",
	NextHint:              "Next: set your API key (export DEEPSEEK_API_KEY=... or add to .env), then run `gaea run \"your task\"`.",
	...
	UsageRunHint:      "usage: gaea run [--model NAME] <task>",
	ErrorPrefix:       "error:",
	...
	UsageBody: `gaea — a config- and plugin-driven engineering office assistant (multi-model)

Usage:
  gaea chat [--model NAME] [-c|--continue] [--resume]   interactive session (multi-turn; -c resumes the latest, --resume picks one)
  gaea run  [--model NAME] [--max-steps N] <task>   run one task and exit
  gaea serve [--model NAME] [--addr HOST:PORT]      serve the session over HTTP+SSE (browser client at /)
  gaea setup [path]                                  interactive config wizard; writes gaea.toml (+ .env)
  gaea mcp <add|remove|list>                         manage MCP servers in gaea.toml
  gaea version
  gaea update [--repo owner/repo] [--tag vX.Y.Z] [--check]   self-update from GitHub
  gaea help

Examples:
  gaea chat
  gaea chat --continue
  gaea run "implement the TODOs in main.go"
  gaea run --model mimo-pro "add unit tests for this function"
  echo "explain this code" | gaea run

Configuration:
  Resolution: flag > ./gaea.toml > ~/.config/gaea/config.toml > built-in defaults
  Secrets come from the environment via api_key_env (e.g. DEEPSEEK_API_KEY).
  Run 'gaea setup' to scaffold a config; see docs/SPEC.md.
`,
}

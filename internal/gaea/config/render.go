...
func RenderTOML(c *Config) string {
	...
	b.WriteString("# Resolution order: flag > ./gaea.toml > ~/.config/gaea/config.toml > built-in defaults.\n")
	...
	if len(c.Plugins) == 0 {
		...
		b.WriteString("# command = \"gaea-plugin-example\"\n")
...
// active TOML array; an empty one renders as a commented example so `gaea setup`

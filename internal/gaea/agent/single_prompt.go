...
- **文档结构**：报告/方案必须先列大纲再填充，最终用 run_skill 调 doc-assemble 技能或已安装的 docx 技能输出完整文档；演示文稿先确认大纲（封面→章节→总结），再用 run_skill 调 pptx 技能输出 .pptx。
...
- **过程/中间文件**（提取的文本、OCR 页图、脚本 .py、临时图表/截图、调试输出、
  中间表格等）一律写入 .gaea/work/<任务名>/，目录不存在先创建；**禁止**
  写到工作空间根目录或与源文件混放。源文件（用户提供的 PDF/docx/xlsx 等）
  保持原位，不要移动或覆盖。
- **最终交付物**（报告 docx/md/pptx/xlsx、成品图表）写入 .gaea/exports/，
  或按用户明确要求写到指定位置。
- 每次写完文件，正文给出相对工作区的可点击链接，方便前端预览。

...
//  1. 请求体形态——首轮带 5 个 tools、次轮把 assistant(tool_calls)+tool 结果接回；
...
func TestSinToolLoopExecutesToolAndPersistsTrace(t *testing.T) {
	...
	// 请求形态：两轮；首轮带 5 个工具定义；次轮接上 assistant(tool_calls)+tool 结果。
	if llm.count() != 2 {
		t.Fatalf("请求轮次 = %d, want 2", llm.count())
	}
	if got := len(sinReqTools(llm.request(0))); got != 5 {
		t.Errorf("首轮 tools = %d, want 5", got)
	}

...
func callHerdsmanTranslate(baseURL, model, system, prompt string) (string, error) {
	...
	resp, err := client.Post(v1Join(baseURL)+"/chat/completions", "application/json", bytes.NewReader(body))
	if err != nil {
...
func (a *App) LocalTranslate(req LocalTranslateRequest) (LocalTranslateResult, error) {
	...
	if src := strings.TrimSpace(req.SourceLang); src != "" {
		fmt.Fprintf(&prompt, "源语言：%s；", src)
	}
	fmt.Fprintf(&prompt, "目标语言：%s。\n\n%s", target, text)
	...
	usedFallback := !found
	if found {
		reply, err := callHerdsmanTranslate(a.herdsmanBaseURL(), model, translateSystemPrompt, prompt.String())
		if err != nil {
			return LocalTranslateResult{}, err
		}
		return LocalTranslateResult{Text: reply, Model: model, Engine: engine}, nil
	}
	...
	reply, err := a.client.ChatSimpleStreamWithOptions(context.Background(), model, translateSystemPrompt, prompt.String(), ai.ChatSimpleOptions{
		EngineID:       engine,
		Temperature:    0.3,
		MaxTokens:      4096,
		TimeoutMinutes: 3,
	...
	if err != nil {
		return LocalTranslateResult{}, fmt.Errorf("回退翻译失败（%s/%s）: %w", engine, orDefault(model, "默认模型"), err)
	}
	return LocalTranslateResult{Text: strings.TrimSpace(reply), Model: model, Engine: engine, UsedFallback: usedFallback}, nil
}
...
func (t translateTextTool) Execute(_ context.Context, args json.RawMessage) (string, error) {
	if t.a == nil {

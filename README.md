...
- ✨ **回答即 UI（GenUI）** — 模型回答可内嵌 genui 围栏：卡片/表格/图表/表单/测验
  渲染成真实交互组件，本地即时判卷/排序/重做，action 回传模型继续对话
...
- 🏗️ **通用办公** — 轻量工具集（17 个内置核心工具 + 技能扩展）：文档转换/图表生成/文档拼装、
  ...
  常用资料固定（新会话自动带入）、任务模板库（欢迎页一键发起 + slash 命令）；GenUI 交互
  面板可投递右侧「UI」会话工作台，由模型 REPLACE/APPEND 原地更新
...
| 版本 | 日期 | 说明 |
|------|------|------|
| **v4.136.0** | 2026-09-07 | 进度计划小刀包：任务路径分析（驱动边 ff=0 判定+前驱/后继链高亮淡化）+任务检查器（「开始由谁决定」结论行+前驱/后继可跳转）+排序与多级分组（合成组头行+折叠，排序/分组持久化）+大纲折叠顺带销项；导出余项 bar 图面标注（任务名（工期）条尾）+里程碑旗标（横道/单代号/双代号三图面）+导出开关；机制蒸馏 ProjectLibre（E8/U3/U1）；纯前端 590 零变更；vitest 2315→2368（+53）、go 绿、drift PASS@590；走查五项全过。详见 releases/v4.136.0.md |
| **v4.135.0** | 2026-09-07 | 进度计划刀C 收口：多级撤消/重做+任务筛选+行右键菜单+进度编辑入口（590 零变更）——store 快照历史（50 上限+文本编辑 800ms 合并+外部回读不入史）+Ctrl+Z/Y；两遍法筛选（关键/手动/文本，分组名命中展示整支）四者同源联动；右键菜单（添加/里程碑/删除，undo 兜底）；+进度列缺省隐藏可编辑；vitest 2301→2315（+14）、go 绿；走查四交互全过。详见 releases/v4.135.0.md |
| **v4.134.0** | 2026-09-07 | 进度计划刀D3：Excel 导入导出（绑定面 588→590）——Go excelize 上报口径 7 列往返：导出 CPM fail-closed+分组跨度+`2FS+3` 前置引用；导入表头别名归一识别+WBS 深度分级+前置引用回链+日期仅取开工日（排程交回 CPM 重算）+Validate fail-closed；工具栏「导入/导出 Excel」按钮；go +7 用例、vitest 2301；走查 导出回执+往返保名。详见 releases/v4.134.0.md |
| **v4.133.0** | 2026-09-07 | 进度计划刀D2：网络图上报件+AOA 逆推锚点修正（588 零变更）——双代号时标网络上报件（工程标尺四行：工程日/月/日/星期+月界竖线，日宽自适应 36~110px/天，波形线/过桥/标注复用刀H 纯函数）+单代号（六格盒/绑定红链/虚拟 S/T）；ExportDialog 图面类型切换；修 aoa.ts 逆推锚点错位（total 误取正推前初始 es，长链假负时差 min(ls) −111→0，planFinish 锚点首次真实生效）；vitest 2291→2301（+10）、go 绿；走查 三图面导出+目检全对。详见 releases/v4.133.0.md |
| **v4.132.0** | 2026-09-07 | 进度计划刀D1：图面导出——横道图上报件（588 零变更，纯前端）——ganttExport 纯函数构建上报图面：标题带+上报 6 列表格（序号/名称/工期/开始/完成/前置）+双行时标/周末底纹+关键红/汇总黑/里程碑菱形/时差尾/四型依赖线/竣工线/今日线+图例+图签；imgPdf 最小 PDF 封装（JPEG 直嵌零依赖）；PNG/PDF/打印三出口+签署字段持久化；vitest 2274→2291（+18）、go test 绿；走查 导出回执+图面目检全对。详见 releases/v4.132.0.md |
| **v4.131.0** | 2026-09-07 | 进度计划刀C：工作台双栏（588 零变更，纯前端）——横道改「表格窗格｜可拖分隔条｜画布窗格」，拖动收纳表格（最小=行号+名称）双击复位、纵向滚动同步；列显隐菜单 12 列单列可藏（行号/名称固定），ganttCols 纯函数+chatPrefs 持久化；时间刻度竖排修复（day-cell nowrap+<10px 不写数）；AI 栏折叠入口进栏头（i18n +3 键）；vitest 2262→2274（+12）、go test 绿；走查 拖窄/复位/列显隐/窄刻度/AI 折叠全过。详见 releases/v4.131.0.md |
| **v4.130.0** | 2026-09-07 | 进度计划基本功刀H：图例语言补全（588 零变更，纯前端）——G2 标注归位（edgeSegs 纯函数，工作名称在箭线上、工期在下，波形时居中于实体段）；G4 时标波形线（auto 改真时标轴 x=最早时间×列宽，水平时差段画波形、虚工作整段波形、手动布局不画波形，图例补波形线/标注口径）；G1 过桥法（findBridgeArcs 竖段垂直穿越他边横段画半圆，端距/间距护栏，双代号/单代号共用，PdmView 首次拥有组件测试）；vitest 2241→2262（+21）、go test 绿（零改动）；走查 35 箭线 10 波形全非关键/2 处过桥/20 组标注 0 错/27 节点真时标。详见 releases/v4.130.0.md |
| **v4.129.0** | 2026-09-07 | 进度计划基本功刀G：引擎诚实化（588 零变更，TS/Go 镜像）——E1 自由时差 SS/SF 搭接漏减前置 ES 锚点（虚高、违反 TF=0⇒FF=0 定理），freeFloatPart 四型口径单测钉死，实证 管道铺设 FF 11→0/沥青面层 2→0；G3 目标竣工作计划工期锚点进逆推（超期时绑定链负时差诚实呈现、关键工作=TF 最小集；manual 驱动场景维持 TF=0 经典口径），ComputeCpmPlan/PlanFinish 接线 UI/基线/摘要/analyze，AOA 事件逆推同步收紧；vitest 2241/2241（+11）、go test ./... 绿（+4 镜像）；走查 deadline 收紧后绑定链 TF=−2/状态栏超 2 天。详见 releases/v4.129.0.md |
| **v4.128.0** | 2026-09-06 | 进度计划基本功刀F：单代号网络图合规整改（588 零变更，纯前端）——布局从时标分层改拓扑分层（layerByTopology：列=最长路径深度，并行分支同列并列，修「整网一条直线、分支全灭」）；多起点/多终点增画虚拟起点 S/完成 T 虚线框节点（JGJ/T 121 口径）；关键线路贯通=两端关键且搭接为绑定约束时红色加粗（isLinkBinding 四型判定，SS/FF/SF 不再漏标）；节点行号角标与横道引用互认；vitest 2230/2230（+6）；走查 16 节点 2 行/红搭接 12 条贯通/S+T 在位/自动适配一屏。详见 releases/v4.128.0.md |
| **v4.127.0** | 2026-09-06 | 进度计划基本功刀A+E+B：呈现层合规整改（588 零变更，纯前端）——刀A 双代号箭线正交画法（横平竖直直角拐弯禁斜线，JGJ/T 121 口径，标注随水平段归位，平行边错位通道）；刀E 视觉纪律（分组行降饱和淡底+左色条让位任务条、次级文字 outline→on-surface-variant 修「15% 透明青」对比度缺陷、网络图坐标纸降强度、图例吸顶、单代号缩放/适配全图）；刀B 横道表格补齐 Project 口径（最迟开始/最迟完成/总时差/自由时差+前置/后续「3FS+2」引用文本+依赖线按 FS/SS/FF/SF 四种搭接取锚点，不再全画 FS 形状；手动任务时差诚实显 —）；vitest 2224/2224（+8）；?mock=demo 走查 DOM 断言（39 条 AOA 箭线 0 斜线/18 条横道依赖线 0 斜线/六时参列齐全）+三视图截图复核。详见 releases/v4.127.0.md |
| **v4.126.0** | 2026-09-06 | 本地模型使用两刀（588 零变更）——①办公切换器模型粒度化：GaeaModels 按引擎展开全部对话模型（本地组置前+运行徽标+展示名，非 llm 过滤），GaeaSetModel 支持 engine/model 精确设默认模型（清单外 fail-closed 报错）；②换模预估按目标模型全覆盖诚实化：herdsman 按目标模型查目录、ollama 查原生 /api/ps+/api/tags（:latest 归一）、modelhub 探测 Studio loaded，未加载不假报秒数、服务不可达如实 unknown（旧版非 herdsman 恒 hot 为 T5-3c 旧账）；vitest 2216/2216（+2）。详见 releases/v4.126.0.md |
| **v4.125.0** | 2026-09-06 | 联动候选：摘要卡「设为当前计划」（588 零变更）——非当前计划一键复制为当前计划（走 fail-closed 通道+即时回读，循环依赖不提供入口）；拍板池三设计文档落档（diff 确认卡/双工期口径/多工程，并行子代理调研）；vitest 2214/2214（+1）。详见 releases/v4.125.0.md |
| **v4.124.0** | 2026-09-06 | 资源成本刀2+刀3：agent 通道+板块 UI（588 零变更）——ops 扩资源四操作+apply 回执强制带总成本+get/analyze 成本叙事与未定价 finding+技能条款（锚点锁 31）；资源工作表弹层+甘特成本列+任务资源 Popover+状态栏总成本（无数据诚实隐藏）；vitest 2213/2213（+19）；?mock=1 全链走查贯通。详见 releases/v4.124.0.md |
| **v4.123.0** | 2026-09-06 | AOA 手动布局刀1（588 零变更）——锚点键（事件业务身份跨拓扑稳定）+ aoaLayout.ts 纯函数（半格吸附/结构归一/混合共存/提交剪枝）+ gsched.json 内嵌 pins（TS↔Go 镜像 pass-through）+ AoaView 双模式开关与拖拽布点（对齐刀9 纪律：未移动不提交/Esc 取消/自动拖拽=转手动）；布局=展示层，buildAoa 引擎口径零变更；vitest 2194/2194（+15）。详见 releases/v4.123.0.md |
| **v4.122.0** | 2026-09-06 | 资源成本刀1：模型+成本 rollup 引擎+持久化（588 零变更）——三类资源+分配+任务 fixedCost（全 optional 旧文件零迁移）、cost.ts/cost.go 镜像引擎（工时=工期×units×费率+每次使用/材料=总量×单价/成本=金额；循环 fail-closed；金额到分）、normalize 容错+Go Validate fail-closed（分组行禁挂分配）；vitest 2179/2179（+13）。详见 releases/v4.122.0.md |
| **v4.121.0** | 2026-09-06 | 进度计划刀12：办公联动三件套（588 零变更）——办公文件预览 `.gsched.json` 摘要卡（工期/关键/基线漂移/倒排+原始 JSON 开关，解析失败回落文本）+「在进度计划中打开」跳转、进度板块「办公侧查看」反向入口、「AI 进度周报」一键发共享线程（运行中 Steer/空闲 Submit）；不并板只联动，跳转/周报限当前计划文件；vitest 2166/2166（+15）。详见 releases/v4.121.0.md |
| **v4.120.0** | 2026-09-06 | 进度计划刀11：左栏细节完善（588 零变更）——宽度拖拽分隔条（320~680 钳位+双击复位）+折叠/宽度 localStorage 持久化（chatPrefs 字段级容错）+新会话 Popconfirm 二次确认+常用指令快捷条 4 chip+头部线程共享副标题；vitest 2151/2151（+5）。详见 releases/v4.120.0.md |
| **v4.119.0** | 2026-09-06 | 进度计划刀10：左 AI 对话·右工作台（588 零变更）——双栏版面（左 420px 对话栏可折叠+右工作台零删减）；与办公板块**共享同一条工作线程**（store 单例）；store 事件绑定提升模块级恰好一次（防 keepAlive 双订阅致气泡翻倍，回归测试钉死）；schedule_apply 回执即时回读+审批/追问卡 pane 内完成；vitest 2146/2146（+1）。详见 releases/v4.119.0.md |
...
| **v4.77.0** | 2026-09-04 | 任务页自动激活补全（绑定面 561 零变更，纯前端）：新后台任务（queued/running/stopping）自动激活「任务」视图；自动激活宽屏≥1240px 才展开、窄屏不强制、沿用设置开关可关；运行中任务按钮改「强制终止」（排队=取消）；子代理实时拓扑/批量预览/退出码/实时输出 dock 已在；vitest 223/1681、tsc/eslint 0、冒烟 200 |
| **v4.76.0** | 2026-09-04 | 任务面板修正（绑定面 561 零变更，纯前端）：「子代理」分组头点击可整体折叠/展开整棵树；AgentTree 卡片圆角显式改 8px；子树加左侧树形连接线；单卡标题点击=折叠、打开对话收敛为卡片独立按钮（subagent.openThread 三语新增）；vitest 223/1680、tsc/eslint 0、冒烟 200 |
| **v4.75.0** | 2026-09-04 | 右侧任务面板卡片化（绑定面 561 零变更，纯前端）：AgentTree 子代理节点改为独立圆角卡片——标题+状态胶囊（进行中/已完成/失败）+模型/工具数/耗时/token 指标行；运行中卡片 glow 描边+实时动作预览、完成卡展示回答摘要、主 agent 主色底；视觉模型验收 92/100；vitest 223/1678、tsc/eslint 0、冒烟 200 |
| **v4.74.0** | 2026-09-04 | 记忆界面技能重设计（绑定面 561 零变更，纯前端）：依 redesign/ui-ux-pro-max 审计重构为 hero 卡 + 事实/文档/建议三统计小卡 + 快速添加卡 + 单张视图大卡（分段/搜索/内容同卡）；TabButton 紧凑胶囊；视觉模型验收 9/10；vitest 223/1678、tsc/eslint 0、冒烟 200 |
| **v4.73.0** | 2026-09-04 | 记忆迁入主区标签页（绑定面 561 零变更，纯前端）：删除左侧「记忆」入口（展开/折叠态），ChatTabs 增「记忆」tab 于上下文旁；Ctrl+K 与 /memory 命令改切主区 tab；记忆界面整体卡片化（头部卡+快速添加卡+胶囊分段+事实独立小卡），抽屉路由收敛；vitest 223/1678、tsc/eslint 0、冒烟 200 |
| **v4.72.0** | 2026-09-04 | 删除主区「概览」tab（绑定面 561 零变更，纯前端）：ChatTabs 收敛为 对话/轨迹/上下文 3 tab；删除 OverviewPanel 容器与命令面板「概览面板」条目；历史 overview 选择自动回落对话；底层 StatsPanel 统计模块暂留未接线；vitest 223/1677、tsc/eslint 0、冒烟 200 |
| **v4.71.0** | 2026-09-04 | 上下文页卡片化（绑定面 561 零变更，纯前端）：8 统计项各自成独立小卡（ctx-tile 2→4→8 列），Token/耗时/会话信息三大仪表卡 + 图标章卡头；事件/文件活动/浏览器节点行改独立圆角卡行（ctx-row）；卡片表面抬升 surface-container + 顶部内高光、零硬编码色值；三语死键清理；vitest 224/1683、tsc/eslint 0、冒烟 200 |
| **v4.70.0** | 2026-09-04 | 上下文页视觉重设计（绑定面 561 零变更，纯前端，对齐 dsh styles.css 实测）：数字层级放大（统计格 15px/当前上下文大数字 26px/环心 16px），当前上下文宽卡强调（分段条 h-4 rounded + 卡头 12.5px semibold），卡头「标题左+副注右」统一，Token/耗时环形图 92px；事件多选 chips、趋势卡内联详情（v4.69 交互）保留；vitest 224/1683、tsc/eslint 0、冒烟 200 |

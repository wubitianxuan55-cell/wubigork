...
| 版本 | 日期 | 说明 |
|------|------|------|
| **v3.8.0** | 2026-09-01 | 双空间内核（工位/乐园隔离：会话/记忆/任务/产物/模型/工具/权限/护栏全按空间装配，space.mode 可回退）+ 质量地基（并发加固/Registry 锁/gate 原子化/retry_until 门控/edit_file 工具层/前端虚拟化与轮询门控/CI -race）+ 长期规划定稿（编程板块保持独立 DSH 窗口）。绑定面 502、Go 115 包、vitest 全绿。详见 releases/v3.8.0.md |

...
> 最后更新: 2026-09-01（v4.27.0「右侧面板对齐 Codex · 子代理对话实时下钻 /
> 对话与上下文完善」：绑定面 550 零变更）

## 当前状态

- **最新发布：v4.27.0（2026-09-01）「右侧面板对齐 Codex · 子代理对话实时下钻 /
  对话与上下文完善」**：git tag v4.27.0；基线 v4.26.1；绑定面 550 零变更
  （纯前端，Go 零改动）。主线=用户连续要求「对齐 Codex 右侧面板与对话输出」，
  本轮四面收口：①右栏文件工作台——点文件预览占满右栏（原顶部 3/5 小窗+底部
  文件树）、文件树收敛为「文件」切换的 260px 侧栏、宽度 720→1600（视口感知
  钳制保聊天区）、首次打开自动抬升 560、编辑器 tab 文件类型图标（lib/fileIcon
  单源）、树内高亮编辑文件；②标签扁平化——删资料/成本库、取消二级标签 →
  文件/产物/变更/任务/分工一级平铺（旧存储值收敛）；③对话输出——用户消息去
  气泡、回合分隔线、助手复制、diffstat 芯片；④子代理实时下钻——点击子代理 →
  全面板对话 SubagentThread（运行中 3s 轮询+事件驱动实时刷新、自动跟随底部）；
  ⑤上下文标签——水位分色总览头部/空态/文件活动可点/占窗口 %/悬停构成详情。
  **验证**：vitest 1082/1082（169 文件）；tsc -b/eslint 0；go build 0 FAIL；
  drift PASS（550）；版本四处 4.27.0。**说明**：规划 v4.27「浏览器与版本」
  顺延；欠账与教训详见 releases/v4.27.0.md 与 .gaea/AGENTS.md。
...
- **模块制市场调研复扫（2026-08-31，调研非刀，v4.11.0 基线，绑定面 544 不变）**：
  ...
  `docs/market-research-2026-08-31.md`（分模块原始稿 `docs/research-2026-08-31/`）。

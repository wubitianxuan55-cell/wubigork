...
> 最后更新: 2026-09-05（v4.87.0「统一 diff 渲染升级：改蓝配对 · 行内
> 字符高亮 · 上下文折叠」，578 零变更，阶段二收口；v4.86.0「Git 面板
> 最小集」，571→578；v4.85.0
...
- **蒸馏规划滚动中**：阶段一（1a/1c）、阶段二.5（2.5a~2.5d）、阶段二
  2a/2b/2c（v4.85~v4.87）**全部收口**。**剩余候选均为拍板项**：2.5e
  /context 弹层（建议与「剩余上下文 % 常驻徽标」合并）、阶段三 3a
  CodeMirror（可带语法着色）/3b Mermaid strict/3c 人工沙箱浏览器多开；
  中期候选见 docs/market-research-2026-09-05.md（成本费率 hover、终止
  级联、失败子代理可恢复）；docx/xlsx 对比迁移 ChangesDiff 查看器另刀。

...
> 最后更新: 2026-09-05（v4.85.0「本轮文件三态折叠：写入/编辑/读取独立
> 成层 + 类型筛选」，571 零变更；v4.84.0「HTML 沙箱预览 · 外链协议分流」，
...
- **蒸馏规划滚动中**：阶段一（1a/1c；1b 勘误销账）、阶段二.5（2.5a/2.5b
  前后半/2.5c/2.5d）、阶段二 2a（v4.85 三态折叠+类型筛选）**全部收口**。
  **下一刀候选**：2b/2c Git 面板与统一 diff（需拍板）、2.5e /context 弹层
  （拍板，建议与「剩余上下文 % 常驻徽标」合并）；中期候选见
  docs/market-research-2026-09-05.md（成本费率 hover、终止级联、失败子代理
  可恢复）。

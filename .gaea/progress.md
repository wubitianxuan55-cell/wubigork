...
> 最后更新: 2026-08-30（v4.6.1 微信统一路由·规范包机制·归因对标发布完成）

## 当前状态

- **最新发布：v4.6.1（2026-08-30）**——「微信统一路由 · 规范包机制 · 归因对标」
  （审计补课续刀）：S4.5 微信接统一路由（routeIntentWithResult 产物感知 +
  微信回调提醒之后先过路由，能力面全量命中）+ iLink 图片消息协议第一刀
  （image_item/file_item 防御解析 + 非文本转模型提示行，未知空项宁漏勿误）；
  规范包机制化（Checker 注册表 + 红头/造价工程表式双检查器，OfficePanel 分组）；
  成本归因对标（明细 vs 参考带宽，差幅等级/贡献金额/主因，参考池排除自身防
  自对标，绑定面 530→531 + FiveCalcPanel 归因区）。验证：Go 全量绿；vitest
  **791/791**；tsc -b / eslint 0；绑定面 531 drift PASS。详见 releases/v4.6.1.md。
- **下一执行**：v4.7——Ctrl+K 命令面板接内核（S4.6 留位）；iLink 文件卡片/
  图片识别真机收敛；成本知识图谱可视化形态；v4.6 原定端到端实时语音继续排期。

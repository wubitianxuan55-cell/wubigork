...
> 最后更新: 2026-09-05（v4.81.0「文件活动行级增量：±徽标 · 操作日志展开 ·
> 详情跳转」，571 零变更；v4.80.0「上下文工具结果深读：完整调用懒加载 ·
> 来源 chip · 分类内排序」，570→571；v4.79.0「上下文对比上一步」，570 零
> 变更；v4.78.0「任务强制终止收口」，569→570；drift 571=权威口径——
> **勘误：v4.67~v4.77 所记 561 为过期口径**（小说并行会话未提交绑定在手）；
> v4.71~v4.77 七版已于本会话补档提交（ae462b20）+ 补 tag；v4.70.0「上下文页视觉重设计：数字层级 · 构成强调 ·
...
- **蒸馏规划滚动中**：1a（v4.78）、2.5a（v4.79）、2.5b 前半（v4.80）、
  2.5c（v4.81）已完成；1b 勘误销账（右键菜单/@引用早已落地）。**下一刀
  候选**：2.5b 后半（图片缩略卡 + 图片 token 估算）、2.5d（Trend step
  brief 跳转 + 设置中心默认粒度/模式/排序）、1c（HTML 沙箱预览 + 外链
  协议分流）。

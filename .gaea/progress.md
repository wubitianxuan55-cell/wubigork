...
- [ ] 会话恢复/重命名补失败提示：`handleResumeSession` / `handleRenameSession` 当前异常会静默
      （归档/置顶/恢复刚补过 toast），统一接入 App 层 toast。
- [ ] 归档会话的「永久删除」加二次确认：与活跃会话删除交互保持一致，避免误删不可恢复数据。
- [ ] `Welcome` 最近会话与侧边栏「项目」视图对齐：改为从 `projectGroups` 派生跨项目最近会话，
      而非沿用旧扁平 `sidebarSessions`（当前仅当前工作区、且被分页截断）。
- [ ] 变更面板补“累计改动次数”与“最近改动”排序，让验收前核对范围更快。
- [ ] 回归测试补：会话删除后三处注册表（标题/目标/置顶）全部清空的 Go 用例。
...
- [ ] 给 `store.ts` 的 reducer/applyEvent 补纯函数表驱动测试（历史重建、turn 收尾、跨轮文本不覆盖）。

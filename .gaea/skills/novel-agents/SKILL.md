...
- 经 `run_skill` 派生子代理时，在 prompt 中指明 `agents/<role>.md` 作为该子代理的角色卡（把卡片全文或路径交给子代理，并按卡片「输入 / 输出契约」附上项目文件路径与任务参数）。
- 触发词：「派架构师」「角色设计」「写第 N 章」「一致性检查」「拆这一章」「查资料」「给我上下文」等，按上表路由到对应角色卡。
- 多角色协作沿各卡「职责边界」的升级路径链式流转（如：素材研究 → 一致性检查）；需要裁决的结论回传主会话。

...
| ⬜ | v4.350 新挂池余项（①③④⑤已随 v4.351.0 销账：启动反闪/overlay 30 处+pre 底/color-scheme/绘梦有界化+上轮 B13/B14/B15/B17 补遗）：②~~MarkdownContent pre 底~~✓ ⑥ForeshadowPanel 行无 memo+行内编辑每键全列表（数百行可达）⑦ModelCenter ctx context 拆分（state/actions 两通道，五 hook 对象字面量 deps 无法建立故辨伪不 memo）⑧novel-workspace.css:1566 selbar z-1080 越过 antd 全部弹层（无叠加路径，加键盘/程序化触发会翻车） |

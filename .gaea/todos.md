...
> 最后更新: 2026-09-16（v4.325.0 t7 收官——分析 V2 面板+标注高亮+AnalyzeChapter 出 Legacy，绑定面 703→704；**小说·MuMu 蒸馏六域前端接线全清**。本表只留**仍开放**项；已完成历史见 .gaea/AGENTS.md 版本速览与 progress.md）

## 当前开放（按域）

| 状态 | 任务 |
|------|------|
| 🔄 | 阶段七·信任地基与复利层（规划 `docs/gaea-stage7-plan-2026-09.md`）：**7.1 信任地基两刀已清**（7.1-1 记忆评测 v4.312.0；7.1-2 路由学习 v4.316.0：任务级账目三维桶 statsFile v3+成本归因 tab+routesuggest 建议制改绑+本地引擎同池；观察池=按任务实例切片留 7.3 联动、bridge 启发式边界、真机走查挂闲置窗口）；**7.2 技能结晶双入口齐装**（7.2-1 演示式录制 v4.313.0；**7.2-2 journal 历史蒸馏已落**（v4.317.0：skilldistill 纯包精确匹配挖掘+建议卡只提示+7.2-1 审阅管道复用+skill_distill_state.json 审计链；**判据②调用计数已收口**（v4.319.0：read_skill/run_skill 工具级计数+skill_stats.json+能力面板可见——「≥5 次」由真实使用积累，两周后清池核数；观察池=回合级成功率另刀、滑窗/子序列挖掘 V2 等真实数据、journal 回放纳入 verdicts 过滤、真机走查一条龙挂闲置窗口））；**7.3-1 收件箱已落**（v4.318.0：taskinbox 纯包状态机+task_inbox.json+四入口存为任务+双空间首页挂点+TaskInboxPanel，零常驻；**欠账=判据②会话级回源**——resumeSession seam 活在 App 树内首页拿不到，Session 已落库数据就绪；观察池=面板 query DOM 捕获清退、任务模板联动、LLM 兜底分类器 save_task 覆盖）→ 稳定一个零功能周 → 7.3-2 板块降视图；归因账本「按任务实例」切片联动点；候选⑤可自证创作过程挂起待拍板 |

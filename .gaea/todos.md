...
> 最后更新: 2026-09-13（v4.278.0 小说域 v2 契约 + 伏笔一致性体检。本表只留**仍开放**项；已完成历史见 .gaea/AGENTS.md 版本速览与 progress.md）

## 当前开放（按域）

| 状态 | 任务 |
|------|------|
| 🔄 | 小说·MuMu 蒸馏六域（规格 `docs/distill/`）：**t1 契约已落（v4.278.0）**、**t2 P0 解析引擎已落（v4.279.0）**、**t2 P1 反推引擎已落（v4.280.0：归一化矩阵/位置对齐/负反馈重试/RTCO 模板/输入节稳定序）**；t2 余项=app 绑定 `NovelImportReconstruct*` + 导入向导 UI（预览→应用、tail 出口）+ 幂等落库（`tasks` 表 `KindBookImport`）与三件套；t3 长程一致性 / t4 情节分析与重写 / t5 角色职业与状态机接线 / t6 提示词工坊（`order` 字段已先行补上）/ t7 前端统一接线——**未开工** |

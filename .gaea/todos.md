...
| 状态 | 任务 |
|------|------|
| 🔄 | 小说·MuMu 蒸馏六域（规格 `docs/distill/`）：**t1 共享契约已落（v4.278.0）**；t2 拆书导入（三级分章/编码链/幂等）/ t3 长程一致性 / t4 情节分析与重写（9 维+版本恢复+坐标反投影）/ t5 角色职业与状态机接线 / t6 提示词工坊 / t7 前端统一接线——**未开工**；交接书 `docs/distill/09-impl-handoff.md`（含 P0 口径纠偏落地记录） |

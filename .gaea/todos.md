...
> 最后更新: 2026-09-21（v4.370.0 工程健康轮：govulncheck+npm audit 漏洞清零+工具链 1.26.6+pnpm-lock 入库+build.bat 显式前端构建；excelize 上游未修留观。前次：v4.369.0 真机走查班第二班。本表只留**仍开放**项）
...
| 🔄 | 前端·性能（v4.365.0 审计后余池，均行为级需专项）：~~①流式分段渲染~~已落 v4.368.0〔findStableCut 导出+全量 fence 扫描盲区修复+ChatRow stable/pending 两段渲染——每 chunk 实际重解析只剩尾段；genui 契约零破坏〕②聊天消息真虚拟化（现尾部窗口 80 条覆盖 95%）~~③StoryStream 行级 memo 重构~~已落 v4.366.0〔SinAssistantRow 行组件〕④ChapterEditor 受控下沉——memo+上报防抖已落 v4.367.0 等效覆盖大半（受控下沉本体留池，编辑卡顿主病灶已除）⑤Gantt 行组件化+memo⑥聊天历史/造价库分页绑定——聊天历史已落 v4.371.0〔ChatMessagesPage 游标式 limit+beforeSeq、绑定面 707、首屏 200 条+loadOlder 120 条/批、MessageList prepend 通知防弹回尾部；造价库全表仍留池〕⑦base64→blob URL 会话资源管理~~⑧聊天组件 memo 化~~已落 v4.366.0〔四组件 React.memo+ChatPage 五回调 useCallback+EMPTY_REPLIES 常量+ChatInspector 统计单趟化；E16 守卫适配 memo 声明形态〕|~~StoryStream 行级 memo~~已落 v4.366.0〔SinAssistantRow 行组件+parseStorySegments 下沉〕

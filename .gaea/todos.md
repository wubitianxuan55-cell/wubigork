...
> 最后更新: 2026-09-15（v4.315.0 书斋首页 v9「案头」重设计+外观模块下拉化——接管上会话在飞刀收口发版；v4.314.0 书斋首页 v8；v4.313.0 会话录制技能 7.2-1。本表只留**仍开放**项；已完成历史见 .gaea/AGENTS.md 版本速览与 progress.md）

## 当前开放（按域）

| 状态 | 任务 |
|------|------|
| 🔄 | 阶段七·信任地基与复利层（规划 `docs/gaea-stage7-plan-2026-09.md`）：**7.1-1 记忆评测已落**（v4.312.0）；**7.2-1 演示式录制已落**（v4.313.0）；**7.1-2 路由学习三线调研已成**（R1 遥测收口=RecordCall 唯一全量无旁路+ChatRequest 加 Feature json:"-" 带外字段先例+statsKey 扩三维 statsFile v3；R2 绑定=featureModelKeys 8 键/routeModel 链零改动红线/routesuggest 纯包+route_suggestions.json+Apply 复用 App.SetFeatureModel；R3 前端=模型中心「成本归因」tab（AttributionSection）+命名避让 GaeaCostAttribution 重名）——契约待主代理定稿后三线并行实现；7.2-2 journal 历史蒸馏（评测依赖已解锁，观察 Letta/Bionic 逼近）；7.3 任务收件箱+板块降视图（7.3-1 收件箱→稳定一个零功能周→7.3-2）；候选⑤可自证创作过程挂起待拍板。插刀：v4.315.0 书斋首页 v9 案头（上会话在飞刀收口） |

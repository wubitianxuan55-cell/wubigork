...
| 🔄 | 内核·Reasonix (dsh) 0.1.6 蒸馏留池（侦察 `docs/gaea-dsh-016-distill-2026-09.md`，五刀 v4.373.0+v4.379 两刀已落地）：**留池 7 项按价值排序**=①锚定式 token 计量（last usage 锚点+差值，需 per-message 估价缓存）②子代理控制面（send_message/interrupt_agent/双向消息/cold-resume，上游 09-16/17 活跃大改等稳定）③Fork 型子代理（父前缀种子）④子代理结构化输出（outputSchema+两阶段捕获）⑤Plan mode 审批门（非 GoalCard，需前端 intent 渲染配套）⑥session-query FTS5 检索（live-preferred corpus+谱系追踪）⑦session-projection-cache 冷启动加速（收益待评估）。**已销号**=spill 泄洪+中断流结构化块保全（v4.379.0 两刀：internal/gaea/spill 内存库+read_spill 工具+[agent] tool_spill 门；终态/预算阻断 assistant+calls+成对结果落库，恢复路径刻意不落）/timeout-policy 域隔离（gaea 工具超时=工具内错误字符串，无外层 deadline 误判通道）/技能目录 digest 重发布+热加载（=reasonix watcher 同判：缓存稳定前缀）/contextBreakdown（ContextView catSystem/catTools 构成视图已在）。**否定结论不跟进**=dsh 无跨 provider fallback/memory 空目录（gaea memory space 领先）/Retry-After 已有/maxConsecutiveWakes 无靶子/工具中止配对已成立 |

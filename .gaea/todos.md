...
> 最后更新: 2026-09-15（v4.316.0 路由学习 7.1-2——任务级账目三维桶+模型中心成本归因 tab+routesuggest 建议制改绑，绑定面 688；v4.315.0 书斋 v9 案头。本表只留**仍开放**项；已完成历史见 .gaea/AGENTS.md 版本速览与 progress.md）

## 当前开放（按域）

| 状态 | 任务 |
|------|------|
| 🔄 | 阶段七·信任地基与复利层（规划 `docs/gaea-stage7-plan-2026-09.md`）：**7.1-1 记忆评测已落**（v4.312.0）；**7.2-1 演示式录制已落**（v4.313.0）；**7.1-2 路由学习已落**（v4.316.0：任务级账目三维桶 statsFile v3+成本归因 tab+routesuggest 建议制改绑+本地引擎同池；观察池=按任务实例切片留 7.3 联动、bridge 启发式边界、真机走查挂闲置窗口）；余=7.2-2 journal 历史蒸馏（评测依赖已解锁，观察 Letta/Bionic 逼近）；7.3 任务收件箱+板块降视图（7.3-1 收件箱→稳定一个零功能周→7.3-2；归因账本「按任务实例」切片联动点）；候选⑤可自证创作过程挂起待拍板 |

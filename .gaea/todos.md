...
| 状态 | 任务 |
|------|------|
| 🔄 | 阶段七·信任地基与复利层（规划 `docs/gaea-stage7-plan-2026-09.md`）：**7.1 信任地基两刀已清**（7.1-1 记忆评测 v4.312.0；7.1-2 路由学习 v4.316.0：任务级账目三维桶 statsFile v3+成本归因 tab+routesuggest 建议制改绑+本地引擎同池；观察池=按任务实例切片留 7.3 联动、bridge 启发式边界、真机走查挂闲置窗口）；**7.2 技能结晶双入口齐装**（7.2-1 演示式录制 v4.313.0；**7.2-2 journal 历史蒸馏已落**（v4.317.0：skilldistill 纯包精确匹配挖掘+建议卡只提示+7.2-1 审阅管道复用+skill_distill_state.json 审计链；**判据②调用计数已收口**（v4.319.0：read_skill/run_skill 工具级计数+skill_stats.json+能力面板可见——「≥5 次」由真实使用积累，两周后清池核数；观察池=回合级成功率另刀、滑窗/子序列挖掘 V2 等真实数据、journal 回放纳入 verdicts 过滤、真机走查一条龙挂闲置窗口））；**7.3-1 收件箱已落**（v4.318.0：taskinbox 纯包状态机+task_inbox.json+四入口存为任务+双空间首页挂点+TaskInboxPanel，零常驻；**欠账=判据②会话级回源**——resumeSession seam 活在 App 树内首页拿不到，Session 已落库数据就绪；观察池=面板 query DOM 捕获清退、任务模板联动、LLM 兜底分类器 save_task 覆盖）→ **7.3-2 已落（v4.332.0 用户提前拍板解锁：任务优先首页+能力视图+回退开关双位，缺省 classic）**；归因账本「按任务实例」切片联动点；候选⑤可自证创作过程挂起待拍板 |
| 🔄 | 小说·MuMu 蒸馏六域（规格 `docs/distill/`）：**t1 伏笔=契约（v4.278.0）+ 分层注入 P1（v4.293.0）+ 分析驱动自动回收 P2（v4.297.0）+ 生命周期清理与统计 P3（v4.298.0）已落**，余项=无（P4 前端 v4.299.0 已落）；**t2 拆书线收官**（v4.278~v4.292：解析/反推任务化/tail 出口/角色 ID 匹配/串联；观察池=反推任务取消绑定/反向角色补建/骨架 AI 丰富化按反馈）；**t3 长程一致性首刀已落**（v4.300.0：prevSummary 最近 10 章窗口+回退链+反重复约束；**t3 长程一致性三刀收官**（v4.300 窗口预算化/v4.302 P1 记忆生产者/v4.303 P2 语义召回进 SceneBible；观察=真机检索质量 threshold 评估）；**t4 C1 分析 V2 化已落**（v4.301.0）+**C3 首刀整章重写+版本库已落**（v4.304.0；余=（**C3 余项全清：partial 局部重写 v4.321.0+版本历史面板 v4.320.0+场景工程整章重写 v4.322.0**：List/Get 补 UI 消费，五绑定全可达）/C4 余项=前端内联高亮随 t7（后端标注层 v4.306.0 已落））；**t5 首刀差分器已落（v4.307.0：七不变量+存活级联+亲密度最长匹配）+第二刀状态回灌已落**（v4.308.0：SceneBible 实体属性回灌零新增管道/create-chapter character_states P1 槽位/降噪 intimacy==50·loyalty==50/存量项目零噪声；组织顶层差分已落（v4.309.0）+职业管理页已落（v4.310.0）+关系图谱升级已落（v4.311.0：graphData 纯函数三类节点四层边+分类筛选+详情浮层，零新依赖）——**t5 角色状态机域五刀全清**；**t6 提示词工坊已清**（v4.323.0 首刀：模板可编辑覆盖层——promptstore 六规则校验+`{{name}}` 渲染+三级解析+五绑定+PromptWorkshopPanel+prompts ×20 元数据；**v4.324.0 t6-C2 模板包导入导出已落**：content_hash 三态+导入真校验+双门落盘/选文件；观察池=项目级 scope/自建模板键/triggers/升级合并交互/风格注入与 novelstyle 打通/模板包签名）；**t7 前端统一接线已清（v4.325.0 收官：分析 V2 面板九维+标注列表/只读高亮+锚点；AnalyzeChapter 出 Legacy 转正——伏笔面板 v4.299/重写建议 v4.305 先后在位，六域前端接线全清；观察池=编辑器 overlay 持久高亮/标注定位编辑器光标/多章对比/情感曲线图形化）** |

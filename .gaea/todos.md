...
| ⬜ | v4.350 新挂池（审计已给 file:line）：①boot 亮色主题启动反闪 ~2s（index.html 静态 splash 硬编码深空底+BootSplash MIN_TOTAL_MS=2100，须内联脚本预读 localStorage 主题分支）②MarkdownContent pre 底 rgba(0,0,0,0.3) 写死暗值（亮主题中灰脏块）③白色半透明 hover 底亮主题失效群（PersonaPicker/imagegen ControlPanel/AssetStudio/AssetLibrary/VisionTrial 等 ~20 处 rgba(255,255,255,0.03~0.05)）④全仓无 --color-scheme 声明（暗主题原生控件残留亮色）⑤绘梦历史无上限渲染+挂载全量回填 dataURL（history.map 无窗口+restoreHistoryImages 逐条转 dataURL，内存/DOM 双无界；对照素材库 120 条+12/页先例）⑥ForeshadowPanel 行无 memo+行内编辑每键全列表（数百行可达）⑦ModelCenter ctx context 拆分（state/actions 两通道，五 hook 对象字面量 deps 无法建立故本轮辨伪不 memo）⑧novel-workspace.css:1566 selbar z-1080 越过 antd 全部弹层（无叠加路径，加键盘/程序化触发会翻车） |
| ⬜ | v4.349 新挂池（审计已给 file:line）：①其余可点 div 键盘化——MemoryHubPage 分类行/ChapterTreePanel 节点/OutlinePanel/ChapterEditor 三钮/FactCard/StatsPanel 等 ~8 处（照 FileTree.tsx:439-452 范式）②`ResizableDrawer` 手写弹层焦点管理（无 `role=dialog/aria-modal`、开不聚焦关不还焦、遮罩无 Esc；对照 ApprovalModal.tsx:73-91）③图标钮可访问名余量（ChatRow 复制/朗读、ChatModeBar 联网开关、SecurityPanel 检测等，Tooltip 不参与可访问名）④reducer 级整店订阅 13 处（`useStore()` 无 selector：MainLayout:414 / HomePage:33 / AppearancePanel×8 等）⑤`initRuntimePolyfill` 同步成本与 vendor manualChunks（待测收益再定） |

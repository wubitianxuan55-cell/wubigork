...
> 最后更新: 2026-09-21（v4.378.0 Reasonix 真身蒸馏第四弹·留池收官：bash 会话状态锚定+memory activation 二维+subject keys；留池 9 项全销号线收官；dsh 池销 3 余 9；前次 v4.377.0 办公记忆/成本库写入纠偏；v4.376.0 压缩救援阶梯+重复调用裁决+skill 目录预算化；v4.375.0 第二弹四刀；v4.374.0 真身蒸馏七刀；v4.373.0 Reasonix 0.1.6 内核蒸馏五刀见 docs/gaea-dsh-016-distill-2026-09.md；前次 v4.372.0 Gantt 缩放 rAF 合并收官；「行组件化+transform 结构改造」留池需拖拽真机回归。本表只留**仍开放**项）
...
| 🔄 | 内核·Reasonix (dsh) 0.1.6 蒸馏留池（侦察 `docs/gaea-dsh-016-distill-2026-09.md`，五刀 v4.373.0 已落地）：**留池 9 项按价值排序**=①spill 主动泄洪（大结果执行时落盘+locator 取回，需配套取回工具）②锚定式 token 计量（last usage 锚点+差值，需 per-message 估价缓存）③中断流结构化块保全④子代理控制面（send_message/interrupt_agent/双向消息/cold-resume，上游 09-16/17 活跃大改等稳定）⑤Fork 型子代理（父前缀种子）⑥子代理结构化输出（outputSchema+两阶段捕获）⑦Plan mode 审批门（非 GoalCard，需前端 intent 渲染配套）⑧session-query FTS5 检索（live-preferred corpus+谱系追踪）⑨session-projection-cache 冷启动加速（收益待评估）。**已销号（v4.378 同日）**=timeout-policy 域隔离（gaea 工具超时=工具内错误字符串，无外层 deadline 误判通道）/技能目录 digest 重发布+热加载（=reasonix watcher 同判：缓存稳定前缀）/contextBreakdown（ContextView catSystem/catTools 构成视图已在）。**否定结论不跟进**=dsh 无跨 provider fallback/memory 空目录（gaea memory space 领先）/Retry-After 已有/maxConsecutiveWakes 无靶子/工具中止配对已成立 |

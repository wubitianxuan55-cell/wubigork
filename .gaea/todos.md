...
| ⬜ | v4.358 审计挂池（辨伪项，附 file:line）：SQL 线=FTS 降级 LIKE 多列长 OR 链疑似踩在册 modernc 空集坑（fts.go:282-353，未确证形状）/SaveVersion 的 MAX+INSERT 并发窗口（costproject.go:293）/migrateLegacyDB 逐文件拷贝活 WAL 库（whisper db/database.go:234）/deferred 事务升级 BUSY（associations.go:77）/SelfHeal 启动期 N+1 点查（cost/repair.go:284）/ImportProjectCharacters 无事务（characterlib/store.go:433）；启动线=Startup 幂等缺口（app.go:344）/filewatch 同步 Walk 大工作区（filewatch.go:107）/prompt 引擎与 ai.Client 双构造+桥接早启窗口（app.go:282/main.go:45）；校验线=GaeaListDir 全盘枚举原语（gaea_listdir.go:50，收紧需先查前端全部调用源）/GaeaConvertToPdf 接受绝对路径（gaea_pdf.go:41，需 Pick/白名单统一设计） |

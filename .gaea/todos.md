...
> 最后更新: 2026-09-19（v4.339.0 t7 观察池「标注定位编辑器光标」：分析面板标注行「编辑器定位」→关面板→编辑器光标选区定位〔rune→code-unit 换算〕；gate 跳他章时入口按章一致条件隐藏；零新绑定 705。前次：非版本刀 todos 对账清账+booksource 进度回调乱序根修；v4.338.0 无参绑定审计收官+死链清理 707→705。本表只留**仍开放**项）

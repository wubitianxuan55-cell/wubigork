...
| 状态 | 任务 |
|------|------|
| 🔄 | 小说·MuMu 蒸馏六域（规格 `docs/distill/`）：**t1 契约已落（v4.278.0）**、**t2 P0 解析引擎已落（v4.279.0：三级分章 + 5 级编码链 + 解析报告/告警）**；t2 余项=AI 反推编排（字段归一化 + JSON 负反馈重试）/幂等落库与 `tasks` 表 `KindBookImport`/导入向导 UI（tail 已实现但仅 full 可达）；t3 长程一致性 / t4 情节分析与重写 / t5 角色职业与状态机接线 / t6 提示词工坊 / t7 前端统一接线——**未开工** |

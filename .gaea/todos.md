...
> 最后更新: 2026-09-15（v4.317.0 journal 历史蒸馏 7.2-2——重复模式挖掘+结晶审阅+审计链，绑定面 691；**7.2 双入口齐装**。本表只留**仍开放**项；已完成历史见 .gaea/AGENTS.md 版本速览与 progress.md）

## 当前开放（按域）

| 状态 | 任务 |
|------|------|
| 🔄 | 阶段七·信任地基与复利层（规划 `docs/gaea-stage7-plan-2026-09.md`）：**7.1 信任地基两刀已清**（7.1-1 记忆评测 v4.312.0；7.1-2 路由学习 v4.316.0：任务级账目三维桶 statsFile v3+成本归因 tab+routesuggest 建议制改绑+本地引擎同池；观察池=按任务实例切片留 7.3 联动、bridge 启发式边界、真机走查挂闲置窗口）；**7.2 技能结晶双入口齐装**（7.2-1 演示式录制 v4.313.0；**7.2-2 journal 历史蒸馏已落**（v4.317.0：skilldistill 纯包精确匹配挖掘+建议卡只提示+7.2-1 审阅管道复用+skill_distill_state.json 审计链；**欠账=判据②结晶技能调用 ≥5 次且成功率可见**——需技能调用计数机制 statsFile 先例另刀；观察池=滑窗/子序列挖掘 V2 等真实数据、journal 回放纳入 verdicts 过滤、真机走查一条龙挂闲置窗口））；余=7.2-2 判据②调用计数小刀（候选）；7.3 任务收件箱+板块降视图（7.3-1 收件箱→稳定一个零功能周→7.3-2；归因账本「按任务实例」切片联动点）；候选⑤可自证创作过程挂起待拍板 |

...
> 最后更新: 2026-09-16（v4.319.0 7.2-2 判据②收口：结晶技能调用计数 skill_stats——read_skill/run_skill 双钩子+skill_stats.json 持久化+能力面板技能行「N 次 · 成功率 X%」，绑定面 696。本表只留**仍开放**项；已完成历史见 .gaea/AGENTS.md 版本速览与 progress.md）
...
| 🔄 | 小说·MuMu 蒸馏六域（规格 `docs/distill/`）：**t1 伏笔=契约（v4.278.0）+ 分层注入 P1（v4.293.0）+ 分析驱动自动回收 P2（v4.297.0）+ 生命周期清理与统计 P3（v4.298.0）已落**，余项=无（P4 前端 v4.299.0 已落）；**t2 拆书线收官**（v4.278~v4.292：解析/反推任务化/tail 出口/角色 ID 匹配/串联；观察池=反推任务取消绑定/反向角色补建/骨架 AI 丰富化按反馈）；**t3 长程一致性首刀已落**（v4.300.0：prevSummary 最近 10 章窗口+回退链+反重复约束；**t3 长程一致性三刀收官**（v4.300 窗口预算化/v4.302 P1 记忆生产者/v4.303 P2 语义召回进 SceneBible；观察=真机检索质量 threshold 评估）；**t4 C1 分析 V2 化已落**（v4.301.0）+**C3 首刀整章重写+版本库已落**（v4.304.0；余=C3 partial 局部重写+场景工程+版本历史列表面板/C4 余项=前端内联高亮随 t7（后端标注层 v4.306.0 已落））；**t5 首刀差分器已落（v4.307.0：七不变量+存活级联+亲密度最长匹配）+第二刀状态回灌已落**（v4.308.0：SceneBible 实体属性回灌零新增管道/create-chapter character_states P1 槽位/降噪 intimacy==50·loyalty==50/存量项目零噪声；组织顶层差分已落（v4.309.0）+职业管理页已落（v4.310.0）+关系图谱升级已落（v4.311.0：graphData 纯函数三类节点四层边+分类筛选+详情浮层，零新依赖）——**t5 角色状态机域五刀全清**；t6 提示词工坊（`order` 已先行）/ t7 前端统一接线——**未开工** |

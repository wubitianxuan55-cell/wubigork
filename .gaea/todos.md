...
| ⬜ | v4.359 挂池清账余量（v4.358 池 11 项→落地 7 刀 v4.359.0，留池 4 项）：ImportProjectCharacters 事务化（characterlib/store.go:433，需 Get/Upsert/Associate 三助手 tx 化中型重构；「只增不改重跑自愈」语义在）/prompt 引擎双构造（app.go:293，毫秒级+agent 构造捕获 eng 指针 SetPromptFS 必须先行的时序风险，不改）/SaveVersion MAX+INSERT 并发窗口（costproject.go:293，需唯一索引迁移+存量去重，待复现）/GaeaConvertToPdf 绝对路径（gaea_pdf.go:41，preview→convert 链有合法绝对路径用途，需 Pick/白名单统一设计） |

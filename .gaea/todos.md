...
> 最后更新: 2026-09-19（非版本刀·todos 对账清账：全表对照 git log/CHANGELOG 逐行核查——关 3 行〔TaskCenter 会话维度=v4.229.0 已落/审计刀A=v4.165.0/knip=v4.268.0 清账〕+改写 2 子句〔t4-C4 已随 t7 收官 overlay 入观察池/造价 §6=v4.209.0 已收官外部数据源不引入〕+删 7.3-1 旧欠账注记〔v4.333.0 已收口〕+补记 7.3-2 缺省翻转候拍板；**过时源曾传染 v4.338 未做段，勿再按其排刀**。同刀根修 booksource 进度回调乱序 flaky〔发射串行化 done 单调末次=终值〕。前次：v4.338.0 无参绑定审计收官+死链清理 707→705。本表只留**仍开放**项）
...
| 🔄 | 小说·MuMu 蒸馏六域（规格 `docs/distill/`）：**t1 伏笔=契约（v4.278.0）+ 分层注入 P1（v4.293.0）+ 分析驱动自动回收 P2（v4.297.0）+ 生命周期清理与统计 P3（v4.298.0）已落**，余项=无（P4 前端 v4.299.0 已落）；**t2 拆书线收官**（v4.278~v4.292：解析/反推任务化/tail 出口/角色 ID 匹配/串联；观察池=反推任务取消绑定/反向角色补建/骨架 AI 丰富化按反馈）；**t3 长程一致性首刀已落**（v4.300.0：prevSummary 最近 10 章窗口+回退链+反重复约束；**t3 长程一致性三刀收官**（v4.300 窗口预算化/v4.302 P1 记忆生产者/v4.303 P2 语义召回进 SceneBible；观察=真机检索质量 threshold 评估）；**t4 C1 分析 V2 化已落**（v4.301.0）+**C3 首刀整章重写+版本库已落**（v4.304.0；余=（**C3 余项全清：partial 局部重写 v4.321.0+版本历史面板 v4.320.0+场景工程整章重写 v4.322.0**：List/Get 补 UI 消费，五绑定全可达）/C4 已随 t7 收官（面板只读高亮+锚点 v4.325.0；编辑器 overlay 内联高亮=t7 观察池，v4.325 有意裁剪〔对 antd 样式同步脆弱〕）〔2026-09-19 对账改写〕）；**t5 首刀差分器已落（v4.307.0：七不变量+存活级联+亲密度最长匹配）+第二刀状态回灌已落**（v4.308.0：SceneBible 实体属性回灌零新增管道/create-chapter character_states P1 槽位/降噪 intimacy==50·loyalty==50/存量项目零噪声；组织顶层差分已落（v4.309.0）+职业管理页已落（v4.310.0）+关系图谱升级已落（v4.311.0：graphData 纯函数三类节点四层边+分类筛选+详情浮层，零新依赖）——**t5 角色状态机域五刀全清**；**t6 提示词工坊已清**（v4.323.0 首刀：模板可编辑覆盖层——promptstore 六规则校验+`{{name}}` 渲染+三级解析+五绑定+PromptWorkshopPanel+prompts ×20 元数据；**v4.324.0 t6-C2 模板包导入导出已落**：content_hash 三态+导入真校验+双门落盘/选文件；观察池=项目级 scope/自建模板键/triggers/升级合并交互/风格注入与 novelstyle 打通/模板包签名）；**t7 前端统一接线已清（v4.325.0 收官：分析 V2 面板九维+标注列表/只读高亮+锚点；AnalyzeChapter 出 Legacy 转正——伏笔面板 v4.299/重写建议 v4.305 先后在位，六域前端接线全清；观察池=编辑器 overlay 持久高亮/标注定位编辑器光标/多章对比/情感曲线图形化）** |

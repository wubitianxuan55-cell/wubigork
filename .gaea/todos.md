...
| ✅ | v4.356 后端审计挂池三组全清（v4.357.0）：①错误吞噬→可见化 4 刀（ListChatEnabled 签名上抛/dag sweep Save×2 warn/turn md 投影 warn/状态标签 Save×2 warn）；AppendTurnTraceToDB 辨伪不升级（whisper 链无 Notice 总线，接线成本>收益）②并发→根修 4 刀（PrepareContinue TOCTOU=接通 release 半成品 continueClaims 单飞/BM25 corpusVer 快照原子/weixin gen 代际防双轮询/realtime Dial CAS 防泄首连）；辨伪不修 2 项（file_backend 双写=设计内自愈语义已在/QuickAdd 锁内 IO=锁刻意串行化读改写，移出=并发丢更新）③删 audit.go+decisions.go ~230 行零调用死代码 |

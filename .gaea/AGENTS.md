...
- **近期三版一行账**：v4.150.0 双工期口径刀1（durationUnit 模型+引擎，TS↔Go 镜像）· v4.149.0 ops 投影+Go/TS 对拍收官（刀D）· v4.148.0 project 整量替换引擎强制确认（刀C）——diff 确认闭环四刀已全落地；拍板池三项（diff 确认卡/多工程/双工期）全部启动或收官，余欠账=双工期刀2~4。

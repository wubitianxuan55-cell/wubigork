...
- **最新发布：v4.141.0（2026-09-07）「进度计划导出预览 · 画布窗口内全览 · 周刻度」**——纯前端 596 零变更。①**导出预览**：ExportDialog 内嵌预览区（弹窗 520→720），`svgWithViewBox` 给构建器 SVG 注入 viewBox 后 CSS 等比缩放（width:100% height:auto），350ms 防抖随图面类型/签署信息/条尾标注刷新，CPM 循环时显「无法生成」诚实态；导出动作仍用原始无 viewBox SVG（PNG/PDF 管线不受影响）。②**画布窗口内全览**：日宽由步进档 [8,14,20,28] 改**连续缩放**（zoom×1.35，钳 2~40px，DAY_W_MIN/MAX 常量），「全览」按钮 fitToWindow=canvasPaneRef.clientWidth−2 ÷ 总天数（钳上限 40）+scrollLeft=0 回起点——长计划不再无限横向拉长；拖拽吸附 wdOffsets 口径不受影响。③**周刻度**：底层刻度随缩放切换（WEEK_TIER_BELOW=12）：dayW≥12 逐日号 / <12 自然周段（周一始跨月不断开，`weekRuns` useMemo，标注起始日 ≥44px「M/D」/≥18px「D」，title=工程周序号+起止），月层恒在顶行——MS Project 缩放切刻度口径。**坑：①jsdom clientWidth=0——fit 用例须 Object.defineProperty(pane,'clientWidth',{value:800}) 注入；②连续缩放后月段宽合计断言改读 style.width（jsdom 无布局）；③全量 vitest 偶发 1 失败为在册 CostProjectsView flaky（单跑绿），新用例 3 连跑稳定。**vitest 2458→**2461**（+3：缩放切周层/全览入窗/导出预览 viewBox）、tsc -b/eslint 0、drift PASS@596、build+冒烟过。走查：预览区 SVG+viewBox 断言/zoomout×2 切周层（3 周段首段 9/7）/fit 800px 入窗回日层。

- **近期三版一行账**：v4.141.0 导出预览+画布全览+周刻度 · v4.140.0 工作台专业化（菜单栏/工具栏三行+双层时标+MPP 导入）· v4.139.0 多工程（每文件一工程+索引+切换器+管理动作）——16 项差距 v4.139 全清后进入体验深化与互通 widening。

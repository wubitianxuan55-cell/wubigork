...
- **最新发布：v4.142.0（2026-09-07）「分级双代号网络图（一级/二级界点衔接）· 横道列头错位修复 · 双代号全览」**——①**分级双代号重做**（用户点名「一级和二级是分开的、整个图都是错的」）：`aoa.ts buildAoa` 此前把分组行（level 0，工期 0、无搭接）当工作转换——每分组退化成 START→n→END 零工期假箭线挂在最左/最右列，与二级实际工作网络割裂成两张图；重做=分组行剔出活动网络（与 PdmView `level>0` 过滤同口径）+**分级汇总箭线**：从其二级子网络**开始界点事件**（子级 startOf 中 nodeEs 最小者）连到**完成界点事件**（子级 endOf 中 nodeEs 最大者），共享事件嵌入同一张图；汇总箭线只进展示边集不进 raws——正逆推/浮时/编号全部只由二级决定，critical 恒 false 深色粗线（--sched-group 同横道汇总条色）；轮廓配对=扁平大纲 level 0 行的子级=其后连续 level>0 行，空分组不出箭线；taskEdge 登记分组 id（横道点分组行高亮其汇总箭线）；导出图面同步（深色粗线+图例「一级汇总（界点衔接二级）」）。②**横道列头压缩错位修复**：`.sched-gantt-row` 是 flex、cell 无 flex-shrink:0——表格窗格窄于列合计时列头行（不在 leftW 包裹层内）被压缩与数据行永久错位；修=列头行显式 width=leftW+`.sched-gantt-cell{flex-shrink:0}`（收纳=裁剪隐藏绝不挤压，用户期望口径）。③**双代号缩放/全览**：141 天计划 110px/天=1.5 万像素宽且 AOA 此前无缩放——镜像 PdmView 刀E 范式（scrollRef+zoom+touchedRef 首帧自动适配一次+−/＋×1.2+全览按钮+百分比），缩放下手动布点拖拽位移 ÷zoom 折算回图面坐标；**坑：TS2448——zoom useEffect 闭包引用其后才声明的 w/h 直接编译错，须把图面尺寸派生提前到钩子前（wEarly/hEarly）**。**教训入册：双代号网络图口径=网络图只画实际工作+分级网络的一级箭线必须与二级子网络界点衔接（共享事件），任何把汇总行画成独立假箭线或直接删掉的方案都是错的——用户两级怒批的真实语义。**vitest 2461→**2465**（+4）、tsc -b/eslint 0、drift PASS@596、build+冒烟过。走查：示例工程 5 条汇总箭线嵌入同一网络（前期准备/基础工程/主体结构/机电与装修/竣工阶段）+自动适配 10% 整网入窗零横滚。

- **近期三版一行账**：v4.142.0 分级双代号（界点衔接）+列头错位修复 · v4.141.0 导出预览+画布全览+周刻度 · v4.140.0 工作台专业化（菜单栏/工具栏三行+双层时标+MPP 导入）——16 项差距 v4.139 全清后进入体验深化与互通 widening。

...
- **近期三版一行账**：v4.155.0 双工期欠账放开（cd 全搭接 SS/FF/SF，§3.2.5 重推零不动点，cdEarliestStart 逆查+直接 ls 下界）· v4.154.0 MPP 导入 Project 2013+ 变体（真机样本五处漂移修复，资源分配暂缺）· v4.153.0 双工期刀4（mspdi DurationFormat 8 互通，真机池待走查）——**拍板池三项全部收官**；观察池=2013+ 资源/分配键位、刀4 真机 Project/WPS/斑马走查、E1 负数对称（动逆推才收口）、list_projects 真机；双工期欠账池剩=导入任务日历转 cd 迁移助手/斑马导入口径探测/等效跨度灰显双读数（按需）。

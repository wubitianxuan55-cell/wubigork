...
- **最新发布：v4.140.0（2026-09-07）「进度计划工作台专业化：菜单栏+工具栏 · 双层时标水平化 · MPP 导入」**——①**命令区三行化**（用户批「随意平铺不正规」）：菜单栏 文件/编辑/视图/任务（命令全量归属：导入 XML/Excel/MPP·导出 XML/Excel/图面·撤销重做·增删任务分组·里程碑/检查器·四视图·示例/清空·工程管理）+工具栏（高频命令分组竖线快捷区，导入/导出收拢 Dropdown，testid sched-import-btn/sched-export-menu-btn）+工程信息条（工程名/开工/目标竣工/日历/基线/资源 | 视图档位 Segmented）；旧平铺按钮零删减收编；工程切换器/管理/AI 栏/办公预览收进菜单栏右端；`sched-header` CSS 类废弃换 `sched-menubar`/`sched-infobar`。②**双层时标水平化**：根因=`.sched-gantt-datehead{flex-direction:column}` 致日列纵向堆叠（时标竖排，用户点名）——改两行结构 `.sched-ts-months`（月份跨列段居中，宽≥76px 出「2026年9月」）+`.sched-ts-days`（逐日号），MS Project 口径自左向右。③**MPP 导入主刀**：`internal/schedule/mpp.go` 解析 MPP9/12/14 子集——OLE2/CFB 用 mscfb 库（go.mod 新依赖）+CompObj 版本探测（skip28→len+应用名→len+格式串；appVer 数字回溯用于 2013+ 偏移）+五原语（Props 16字节头+条目奇对齐 / VarMeta9 8字节条目 uid3+type1+off4、VarMeta12/14 12字节 / Var2Data size前缀 / FixedMeta 条目47·37·34·10、count=(len-16)/条目 / FixedData meta 偏移定位行、任务行 cap768）；任务偏移 9/12 相同（Name=var11/WBS=var10、OutlineLevel@40、Duration@60 十分之一分钟、Milestone=metaByte8&0x20、PCT@122）、14 自成一套（Duration@42/PCT@90/Name=var14，顺序键在 Fixed2Data 未读=按 ID 序）；**链接在 TBkndCons 20字节行**（constraintID 严格递增去重、类型 0=FF 1=FS 2=SF 3=SS、lag 有符号@16、2013+ 移@14）；分配 MPP9 定长142/MPP14=110/meta 定位、删除判定=meta 首字节非 0（int32 全非零会误杀真行——踩过）；幻影行=行长<75%×字段表最大偏移（9/12 基准 264=BaselineFixedCost@256+8，**不是** 768 拷贝上限——首版踩坑全跳真行）；口令 XOR（mask=0xFF-加密码）与打开口令 fail-closed；时间戳=1983-12-31 epoch 天数（days<100/65535=空）；Summary 不读文件按「有子任务」重算、两级大纲压平。**坑：流名前导空格**（工程目录 "   19"/"   112"/"   114"）+mscfb Name 不含  前缀；**无压缩**（网上布局文档的 DirIndex/LZW 属另一内部布局，勿实现）。绑定 +1（GaeaScheduleImportMpp，绑定面 595→596，drift PASS@596）；mock 诚实测抛「需桌面端」；前端 api.importScheduleMpp+文件名兜底工程名。实证=ProjectLibre 样例三件真实 .mpp 全链路（Commercial construction 146 任务/176 搭接/158 分配/总工期 344 天 CPM 过）。

- **近期三版一行账**：v4.140.0 工作台专业化（菜单栏/工具栏三行+双层时标+MPP 导入）· v4.139.0 多工程（每文件一工程+索引+切换器+管理动作）· v4.138.0 自定义包（自定义字段 5 槽+编制说明注+项目模板+签证台账）——**进度计划对标 Project 16 项差距 v4.139 全清**，v4.140 起为体验深化与互通 widening。

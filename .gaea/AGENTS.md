...
- **下一执行**：**v4.5「指令中枢」**（2026-08-30 规划修订插入 §10.4a/阶段 4）——
  统一意图→能力路由内核（语音/微信/命令面板同内核多入口）：v4.5.0 = intent 解析包 +
  能力注册表 + 语音指令通路（JARVIS 一档）；v4.5.1 = 微信接统一路由 + 图片/文件卡片；
  v4.6 = 端到端实时语音（云端 Realtime 档）。修订背景：v4.3 后半至 v4.4 滑向小步打磨，
  §8 革命性跃升（实时语音/语音控制桌面/微信完整任务入口）未实质落地，经审查重定主轴。

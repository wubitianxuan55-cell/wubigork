...
## 版本状态
  ...
  - 最新发布：v2.14.11（2026-08-13）「小说板块后端 + 绘梦生成链路闭环」：
    小说补齐章节流式/状态/书架/世界观/统计/导出后端；绘梦补齐生成队列/取消、
    历史元数据持久化、ComfyUI 进度、模板/绘照、LoRA 动态加载/重试。
    绘梦 UI 重构已立项（docs/gaea2/2026-08-13-绘梦板块重构设计.md），下个会话执行。
    详见 releases/v2.14.11.md。
  - v2.14.10（2026-08-13）「修复办公模型改绑不生效」：办公主 agent 模型
    由 bridge 在 GaeaInit 注入并缓存；改绑「办公」后需重注入 bridge + 重建 controller。
    详见 releases/v2.14.10.md。
  - v2.14.9（2026-08-13）「聊天板块后端：原子落库 + AppendMessage 收敛」：
    AppendMessage 用 RETURNING 收敛 seq 分配，AppendExchange 单事务原子写入，落库失败记日志。
    详见 releases/v2.14.9.md。
  - v2.14.8（2026-08-13）「聊天板块后端：会话列表查询收敛 + GetTopic」：
    ListTopics 预览 N+1 收敛为单条相关子查询，新增 GetTopic，创建/导入/导出按 ID 读取。
  ...
  - v2.14.6（2026-08-13）「聊天板块会话导出为 Markdown」：
    ChatTopicExportMarkdown 导出话题为 .md 到用户数据目录 exports/chat，前端一键导出。
    详见 releases/v2.14.6.md。
  - v2.14.5（2026-08-13）「聊天板块真实流式输出（普通对话）」：
    普通对话改为后端逐块流式（ChatStreamPlain + chat-stream:<runID> 事件），
    角色模式保持整段返回；AI 客户端新增 ChatStreamChunks。详见 releases/v2.14.5.md。
  - v2.14.4（2026-08-13）「聊天板块收口：联网搜索污染修复 + 回到底部 +
    侧栏预览同步」：搜索注入不再写入用户历史、上翻「回到底部」、侧栏预览随发送/清空同步。
    详见 releases/v2.14.4.md。
  - v2.14.3（2026-08-13）「聊天板块补强：输入法防误发 + 快速切话题竞态修复 +
    模式栏收敛」：IME 组合态 Enter 不再误发、话题载入竞态令牌、模式栏合并；
    前端用例 190→194。详见 releases/v2.14.3.md。
  - v2.14.2（2026-08-13）「聊天板块优化：会话搜索 + 最近活跃排序 +
    智能滚动 + 重开加载修复」：会话最近活跃排序 + 相对时间、侧栏会话搜索、
    生成中智能滚动（上翻不吸底）、重进聊天正确载入历史消息、重命名失败提示；
    前端用例 182→190。详见 releases/v2.14.2.md。
  - v2.14.1（2026-08-13）「办公板块缺陷收口 + 测试补强 + 结构收敛」：
  收口会话恢复/重命名失败提示、归档删除二次确认、欢迎页跨项目最近会话、变更面板
  汇总排序、归档会话搜索；前端用例 138→179，办公前端首轮结构收敛，收敛 bridge
  动态/静态 import 混用告警。产物 gaea-v2.14.1.exe（31.7MB）。详见 releases/v2.14.1.md。

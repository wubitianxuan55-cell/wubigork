...
- **下会话待办（2026-09-05 收档，详见 progress.md「下会话续做」）**：
  **蒸馏规划滚动中**——1a/2.5a/2.5b 前半/2.5c/2.5d 已完成（v4.78~v4.82），
  下一刀候选：2.5b 后半（图片缩略卡+图片 token 估算，Anthropic 官方公式
  ⌈w/28⌉×⌈h/28⌉ 已核实）、1c（HTML 沙箱预览）、2b/2c Git 面板（拍板）；
  2.5e /context 弹层待拍板。本会话另落 GitHub 市场调研三路
...
- **最新发布：v4.82.0（2026-09-05）「上下文趋势跳转浏览器：brief 行锚点 ·

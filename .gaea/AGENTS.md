...
- **最新发布：v4.265.0（2026-09-13）「原罪画廊重新生成入口」**——v4.263 下刀候选清账；流内插图本有「重新生成」（SinIllustration caption 钮），本刀把同一能力带进右栏画廊，**零 Go 零新绑定**（SinIllustrate 既有全链路：角色锚点+参考槽+退回纯文本+台账+`arts[cue]=path` 覆盖回写）。**落地**=①collectIllustrations 条目扩 `messageId/cue`（SinIllustrate 回写定位）②画廊缩略图悬浮「重新生成」钮（默认藏 hover/focus 显形；busy=「重新生成中…」状态条+缩略图调暗；prompt 空=不出钮——SinIllustrate 拒空提示词；sending 中禁用）③大图 Modal footer 同款钮+重生成成功后预览跟新条目④页面回调=与流内同一 `illustrationQueue` 串行（一次一张）→ SinIllustrate → `reloadMessages()`（useSinStory 新暴露）全链刷新⑤**外部换图采纳**：SinIllustration 记 `extPathRef`，pathProp 变化且自己不在生成中才采纳——防「画廊新图/流内旧图」不同步，也防把自己在途产物顶掉。失败如实 notice（原图保留：回写只在成功路径发生）；`persisted:false`（图已生成回写失败）也如实提示。**测试**=vitest +8（画廊悬浮钮回调定位/busy 挡重复/sending 禁用/空 prompt 不出钮/Modal footer+预览跟新/collect 字段/外部换图采纳+生成中不采纳/reloadMessages）。**门禁**=ci.ps1 全绿/drift OK@641/版本三处 4.265.0。**真机走查**=临时故事端到端（结束即删，用户故事未动）：一轮出正文+插图标记→自动出图落库→画廊重生成 busy 态→回写换新路径+画廊/流内同步换图，零前端错误；产物=exe 49,257,984B SHA256=868668BD…65AB（桌面副本同哈希，冒烟 200 过）。

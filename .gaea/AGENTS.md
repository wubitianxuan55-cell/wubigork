...
- **下会话待办（2026-09-04 收档，详见 progress.md「下会话续做」）**：
  实机验收 v4.62.2～v4.64.1 六版（主对话实时过程/三路并行/卡片跳转/
  子代理 tab 同款渲染/mt_ 有界+拆包/Side Chat 追问）；dsh 剩余借鉴
  （后台任务实时输出+强杀+退出码、追问会话提升为新会话、自动展开偏好
  设置 UI）；SubagentsPanel/Sidebar 迁共享 subagentRunsStore。

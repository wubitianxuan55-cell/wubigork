...
- 最新发布：v2.14.0（2026-08-13）「办公板块会话化升级：项目分组 + 会话生命周期 +
  任务目标 + 变更面板」：侧边栏按项目聚合会话，活跃会话支持置顶/归档/恢复，
  会话首条消息自动锚定为任务目标（需求→验收），右侧新增「变更」面板汇总文件改动，
  新增专注模式；同时修复会话删除注册表清理不完整、记忆图谱三元组被成本节点挤掉、
  App 层 toast 静默失效。详见 releases/v2.14.0.md。

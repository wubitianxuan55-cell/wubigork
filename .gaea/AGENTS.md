...
- **进行中工作树快照（2026-09-03，未发版，两刀）**：办公按 dsh-better-sidebar
  语义 pane 化移植。快照一（d856353e）：右栏欢迎卡 → 视图/文件/浏览器/任务
  tab + 点子代理开独立会话 tab，旧两级组件删除。快照二（本工作树，未提交）：
  **左栏子代理会话入口完成**——Sidebar 会话行可展开出子代理子行（状态点/任务
  ...
  刷新；mock 子代理按父会话归属；三语 +7 键。前端 vitest 210 文件/**1523**
  用例绿，tsc/eslint 0，drift OK；Go 零改动（未跑）。实拍与记录：

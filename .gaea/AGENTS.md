...
- **近期三版一行账**：v4.152.0 双工期刀3（板块 UI：单位 chip+拖拽自然日回写+等效透明）· v4.151.0 双工期刀2（agent 通道：ops durationUnit+cdTasks+技能条款，golden 61 例对拍）· v4.150.0 双工期口径刀1（durationUnit 模型+引擎）——余欠账=刀4 mspdi 互通（绑真机池）。

...
- **近期三版一行账**：v4.149.0 ops 投影+Go/TS 对拍收官（刀D）· v4.148.0 project 整量替换引擎强制确认（刀C）· v4.147.0 diff 确认卡刀B（审批卡结构化 diff 预览）——diff 确认闭环设计四刀全落地，AI 改计划=事前 diff 卡（project 强制/ops 投影）+事后一键回滚。

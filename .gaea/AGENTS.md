...
- **最新发布：v4.139.0（2026-09-07）「进度计划多工程：每文件一工程+索引 · 切换器 · 管理动作 · agent 跟随」**——**16 项差距清单最后一项 #15 收官**（v4.110 挂账 9 个月清偿）；按已拍板设计 docs/gaea-schedule-multi-project-design-2026-09.md 刀1+刀2 一版做完。**绑定面 590→595（+5）+签名扩展 1 处**（GaeaScheduleSave 加 rel——签名级漂移不破方法名口径，GaeaListDir 先例；drift PASS@595）。**internal/schedule/index.go**：索引 .gaea/schedule/index.json=缓存与指针非权威（缺失/损坏/漂移→目录扫描重建宁重建勿报错+Load 轻扫收编游离文件 name 回填）；LoadScheduleIndex/SaveScheduleIndex（原子写）/SetCurrentSchedule（校验+拒 .. 逃逸）/SafeSlugName（中文保留/冲突 -2/60 字上限）。**绑定 +5**：Load 签名不变改读索引 current（索引不可用回落 DefaultRelPath=老用户零感知）；Save(projectJSON, rel) 显式带装载 rel（结构性消解切换竞态，rel 空=当前指针 wails 缺参兜底）+保存刷新 updatedAt；Projects（摘要 name 必真/坏文件 Ok=false）；ProjectOpen（校验+原子切指针不搬数据）；ProjectCreate（slug 落盘全套校验→登记→自动切当前，**文件名此后不改**=agent path 引用稳定）；ProjectArchive（索引标记文件不动可反归档）；ProjectDelete（物理删+摘除，删当前级联切剩余第一个**未归档**无剩余回落缺省落空工程）。**agent 跟随**：resolveSchedulePath 缺省改读索引 current（「对话改的=板块打开的」跨多工程保持），三工具 schema 零扩容，compact 文案同步；list_projects/analyze 跨工程对比维持不做（§3.5）。**store**：currentPath 状态+openProject（dirty 先冲刷旧 rel→Open→重水合：past/future 清空=新工程新历史、selectedId 置空、失败 fail-closed 原状）+create/archive/deleteProject+refreshProjects（冲刷逻辑全动作覆盖）；自动保存显式带 rel；persist 结构不变（降级为当前工程缓存既定取舍）。**UI**：页头切换器 Select（未归档项当前高亮；**1 项收窄纯显示/0 项隐藏=老用户零感知**）+管理 Popover（列表/归档反归档/删除 Popconfirm/新建/当前工程行内改名）；状态栏 title 读 currentPath；**办公卡「设为当前计划」复制语义废弃改切指针**（ProjectOpen+notify 重水合+NAVIGATE——消灭同源文件漂移隐患），打开扩任意计划。**mock**：Map+索引指针同构+5 方法+__mockScheduleFile.list()/open(rel)。**坑：①OfficeB 新委托先落 gaea_schedule_file.go 尾部（包编译依赖）——集成时剪切归位 bindings_office.go 勿两边都留；②bindingNames.ts 与 gen_bindings -names 比对要用 tr -d '' 归一 CRLF 再 diff（否则全行假差异）；③浏览器走查 localStorage/上一版注入残留仍在——每轮走查前清态确认当前计划内容；④antd Select 选项用 mousedown+click 双事件派发才稳；⑤归档项不进下拉但可被级联切中——Select 显示 value(rel) 而非 label 的边角，mock Delete 级联已补归档过滤与 Go 对齐。**vitest 2428→**2455**（+27）、go +15（index 7+app 8）全量 116 包绿、tsc -b/eslint 0、drift PASS@595、build+冒烟过。走查：单工程收窄显示/新建「办公楼二期」自动切换（指针+空态三证据）/双工程互切数据随动/归档下拉消失/删当前级联切剩余+空态；Save rel=前端全显式+Go 两态单测+rel 空串回落兜底（设计风险 1 缓解）。**下一步：16 项差距全清收官；观察池=AOA 长计划画布缩放/E1 负数对称+哨兵（动逆推收口）/刀3 agent list_projects 视实测/多工程资源池+mspdi 导入为新工程入候选池。详见 releases/v4.139.0.md。
- **最新发布：v4.138.0（2026-09-07）「进度计划自定义包：自定义字段 · 编制说明注 · 项目模板 · 签证台账」**——绑定面 **590** 零变更（xlsx.go 内部扩展不改方法签名；差距 #14+PDF 余项 3/4+**#10 模板自设计**+签证台账——**PDF 对比余项全清，16 项差距仅剩 #15 待拍板**；契约先行第三轮：customFields.ts 注册表+Go 镜像，四子代理并行）：**自定义字段**=CUSTOM_FIELDS 5 槽（text1..3/num1..2，key 稳定旧文件零迁移）+project.customLabels 列名覆盖（customLabelOf 缺省回落）+task.custom 只存有值槽；横道 20 列（自定义列缺省收起 DEFAULT_HIDE 追加）、叶行直编（清空=删槽键）、列菜单改名铅笔（表头/菜单/xlsx 列名同源，setCustomLabel 入 undo）；**Go xlsx 往返**=Task.Custom/Project.CustomLabels 新字段，导出 7 标准列后追加有值槽（无值槽不出列），导入剩余表头按 xlsxNorm 匹配缺省名+覆盖名（number 槽 ParseFloat 非法静默丢弃），ImportXlsx 变参 labels 既有调用点零改动，go +4 用例全量绿；**编制说明注**=exportMeta.notes（多行 600 上限）+ExportDialog TextArea+三图面图签上方「编制说明」块（≤6 行逐行 fitText CJK 截断、淡底细边框、块高计入画布图签下移、空 notes 输出逐字节不变）；**项目模板**=templates.ts（localStorage gaea.schedule.templates 上限 10 FIFO 同名覆盖坏条目容错）+工具栏「模板」Popover（另存缺省名=项目名/载入 Popconfirm+importProject 可 undo/删除）；**签证台账**=BaselinesPanel「导出台账」CSV（﻿ BOM，列=名称/时间/基线工期/当前/漂移，无槽隐藏，downloadBlob 复用）。vitest 2399→**2428**（+29）、go +4 全量绿、tsc -b/eslint 0、drift PASS@590、build+冒烟过。走查：列菜单 5 列+铅笔、「文本1」改名「施工部位」表头菜单同步、勾选出列+`sched-custom-text1-A` 填值、TextArea 三行→PNG 下载「已导出」、模板保存「雨污标准段」列表出现、基线槽后台账 CSV 下载触发。**坑：①python heredoc 写 JS 字符串里的 
 会被双重转义成真换行（Unterminated string literal）——多行占位符直接写单行文案绕开；②IAB 走查 localStorage 有上一版注入残留（ganttHide 含 text1/mock 计划被上版覆盖）——走查前 removeItem 关键 key+确认当前计划内容，别拿残留当 bug；③antd Checkbox testid 落 input 本体而 Input.TextArea 落 textarea 本体，原生 setter 要分 HTMLInputElement/HTMLTextAreaElement 取；④IAB waitForEvent("download") 可稳定捕获 CSV/PNG 下载（walkthrough 断言下载触发即可，文件内容由组件级测试钉死）。**下一步：16 项差距收官——仅剩 #15 多工程（设计已落档待拍板）；观察项 AOA 长计划画布缩放/适配；E1 负数对称+哨兵（动逆推时收口）。详见 releases/v4.138.0.md。
- **最新发布：v4.137.0（2026-09-07）「进度计划资源包：资源使用视图+超载检测 · 资源级日历 · 多基线槽」**——绑定面 **590** 零变更（纯前端；差距 #12/#13/#11 一版销三项；机制蒸馏 ProjectLibre E5 工时区间/U6 分桶/E3-E4 可用性收紧/E7 快照槽位；**并行子代理范式第二轮：先钉类型契约 usageTypes.ts 再放 4 子代理**——A usage.ts 10 用例/B UsageView 6/C BaselinesPanel 10/D ResourcePanel +5，零冲突）：**使用视图**=store ScheduleView +'usage' 第四档，computeUsage(project,cpm)（时间轴=项目工作日 wd 与 es/ef 同口径；负载=Σ在工任务 units（es≤wd<ef）；可用性=maxUnits；超载=负载>可用性+1e-9；只有 work 资源参与=材料按总量/成本按金额无按天语义诚实不做假分布；cpm fail-closed），UsageView 左列资源+超载天数红标+右侧逐日热力格（蓝深浅/超载红底描边红点/title 归因「负载 X / 可用 Y（任务×units）」/汇总行/空态/>62 天每 10 标数）；**资源级日历**=SchedResource.calendar?（周制+节假日），ResourcePanel work 行「日历」编辑器（周制至少一项/节假日 Tag/跟随项目日历清除/未自定义跟随项目态），**诚实口径写明：个人休假只影响使用视图可用性与超载判定，不改 CPM 排程**；**多基线槽**=project.baselines[]（上限 3 FIFO 同名原位更新）+baseline 仍为活跃指针（Go baselineDrift/AI 工具/横道基线条口径零漂移），store 新动作 activateBaseline/removeBaseline（走 pushHistory('baseline') 可 undo），BaselinesPanel 换装（槽位 radio 切活跃/当前对比高亮/删槽 Popconfirm/保存命名缺省 基线N/存满 3 提示淘汰/漂移摘要沿用旧口径/清除对比只清指针槽位保留）。**顺带修根因**：store.upsertResource 字段白名单收录 calendar（仅 work 保留+周历逐项容错）——子代理 D 发现白名单静默丢字段先打补丁、主线修 store 后撤补丁（根因修在 store 不在组件）。normalizeProject 靠 spread 天然透传新字段（实测 calendar/baselines 穿透）。vitest 2368→**2399**（+31）、tsc -b/eslint 0、go 绿、drift PASS@590、build+冒烟过。走查（?mock=demo+__mockScheduleFile.save 注入三任务两资源工程）：使用视图 汇总「共 2 名工时资源·峰值负载 2·超载 5 资源-日」+超载格 title 逐格归因正确+示例无资源空态+状态栏第四视图；基线槽 保存两槽/切换活跃/当前对比高亮迁移/漂移持平/3 槽提示；资源面板 2 work 资源=2 日历钮（material/cost 无）。**坑：①upsertResource 白名单丢字段类 bug——schema 加字段必须同步查 store 清洗白名单与 normalizeProject 两条路；②antd Popover 在走查中反复真点会 toggle 关开打架（按钮是 toggle），组件级测试兜底编辑器交互；③删旧组件时连带类型声明（ExportKind）被误删——tsc 一次抓出；④mock 注入后必须 window.dispatchEvent(focus) 才触发回读（在册钩子语义复证）。**下一步：#14 自定义字段（X2 字段注册表+固定槽位）→PDF 余项（编制说明注/签证台账）→#10 项目模板自设计；#15 多工程待拍板；E1 负数对称+哨兵（动逆推时收口）。详见 releases/v4.137.0.md。
- **最新发布：v4.136.0（2026-09-07）「进度计划小刀包：任务路径分析 · 任务检查器 · 排序与多级分组 · 导出标注旗标」**——绑定面 **590** 零变更（纯前端；一版销五项=差距 #8/#9/#16+PDF 余项 bar 标注/里程碑旗标，顺带销项大纲折叠；机制全部蒸馏 ProjectLibre docs/gaea-schedule-projectlibre-distill-2026-09.md 的 E8 驱动判定/U3 合成组头/U1 bar 标注管线，四纯函数/组件由并行子代理产出+主线集成）：**pathDriver.ts**（逐依赖自由时差复用 freeFloatPart，ff=0 且后继 auto=「驱动依赖」；drivingChain 沿驱动边 BFS 前驱/后继链——手动任务可作链端点不再上溯、分组行不进链、循环 fail-closed 空）+GanttView 工具栏「路径｜前驱链｜后继链」Segmented（锚点=选中行；链外行 .sched-row-dim 淡化、链上依赖线描红加重 .sched-link-chain、链外线 .sched-link-dim）；**TaskInspector.tsx**（右键菜单「任务检查器」→右侧 Drawer：概要+六时参（日期换算 MM-DD (+N)）+「开始日期由谁决定」结论行（单驱动可点名跳转/多驱动/手动锁定/无前置四种口播）+前驱后继表（可点跳转+类型时距+自由时差+驱动红 Tag）；SchedulePage 持有 inspectId+depFreeSlacks/drivingLinkKeys useMemo 数据面）；**ganttGroup.ts buildGanttRows**（筛选→排序→多级分组→折叠全部合成进唯一可见行序列，缺省恒等原顺序零回归；排序六键组内稳定重排组头不动；分组三字段至多两级合成组头行（计数+min ES~max EF 汇总条），空分支不产出；WBS 组头获得折叠=大纲折叠销项；排序/分组持久化 chatPrefs ganttSort/ganttGroup 字段级容错）；**ganttExport barLabels**（叶条尾「任务名（N天）」CJK 截断+超右界回退条内白字；ExportDialog Checkbox 缺省开）+**里程碑旗标 expFlagSvg**（横道菱形右侧/单代号六格盒右上/双代号完成事件圈右上按事件去重）。vitest 2315→2368（+53：pathDriver 18/TaskInspector 7/ganttGroup 17/ganttExport +4/networkExport +2/GanttView 集成 +5）、tsc -b/eslint 0、go 绿、drift PASS@590、build+冒烟过。走查（?mock=demo）：路径=水稳前驱链 4 边描红+链外 11 行 13 线淡化、折叠 ▲/▶ 往返、分组 关键:是 8（恰为关键链）/否 8、检查器结论行「由前置【路基整形】(FS) 驱动」+后继 FS+20/SS+6 驱动 Tag、标注 Checkbox 缺省勾选。**坑：①GanttView 行管线换 buildGanttRows 后 `isGroupRow` 不再使用（kind 判别）——漏删会 eslint unused；②antd Checkbox 的 data-testid 落在 input 本体（checked 断言直接取 testid 元素）；③走查右键菜单合成 MouseEvent 这次未触发 antd Dropdown（v4.135 在册方法失效）——Playwright 原生 right-click 反而稳（与在册结论相反，按当前版本实测为准）；④IIFE 包 map 时 return 括号配对易错——tsc 一次抓出。**下一步：差距余项 #12 资源使用视图+超载（E5 区间枚举+U6 分桶直方图）→ #13 资源级日历（E4 交集物化）→ #11 多基线槽（E7）→ #14 自定义字段（X2 注册表）→ PDF 余项最后两件（编制说明注/签证台账）；#10 项目模板自设计；#15 多工程待拍板。详见 releases/v4.136.0.md。
- **最新发布：v4.135.0（2026-09-07）「进度计划刀C 收口：多级撤消/重做 · 任务筛选 · 行右键菜单 · 进度编辑入口」**——绑定面 **590** 零变更（纯前端；刀C 余项四件全清=官方「多级撤消/筛选视图/更新进度/上下文菜单」）：**store.ts 快照式 undo/redo**（past/future 上限 50 不入持久化=会话态；**文本编辑 800ms 合并窗口**——pushHistory 同 (kind,id) 连续编辑只压一次栈，undo 一次回整段打字前；整体替换（导入/示例/清空）kind 缺省强制压栈；外部回读/水合走 setState 直改天然不入史；resetHistorySession 导出供测试隔离）+工具栏按钮（禁用态随历史）+Ctrl+Z/Y/Shift+Z 键盘（activeElement 是 INPUT/TEXTAREA/contentEditable 时不抢）；**ganttFilter.ts 两遍法行筛选**（全部/关键/手动+名称文本不区分大小写；第一遍=分组名命中的分组其全部子孙进强制集「搜支名看整支」，第二遍=叶按模+文本、分组任一直接子孙可见或被强制才保留；返回可见行**原索引**）；GanttView 筛选 UI（Segmented+搜索框）+表格/条形/依赖线（两端均可见才画、y 按可见行序）/前锋线同源联动+空态提示；**行右键菜单**（antd Dropdown trigger contextMenu 包表格行+条形行：在下方添加任务/添加分组/设为取消里程碑仅叶/删除——undo 兜底）；**进度列**（ganttCols 15 列 +progress 56px 可隐藏；叶 InputNumber 0-100，group 留空=汇总唯一口径子孙求和；条形进度覆盖/前锋线既有口径；**缺省隐藏**=chatPrefs DEFAULTS ganttHide ['progress']，parse 缺字段回落 DEFAULT_HIDE）。vitest 2301→2315（+14：store 历史 6/ganttFilter 5/GanttView 交互 3）、tsc -b/eslint 0、go 绿、drift PASS@590、build 2m10。走查：按钮状态机（初始禁用→加任务启用→撤销回落→重做可用）、筛选 20→18→搜「验收」2→清空恢复、右键分组行无里程碑项/叶行设里程碑+撤销回落、进度格组件级钉死。**坑：①antd Input 的 data-testid 落在 input 本体、InputNumber 落在包装层——测试取内部 input 要兼容两种落点；②浏览器走查 antd 菜单用合成 MouseEvent dispatch 可靠（React 合成事件响应原生冒泡），Playwright 真实 right-click 会跟残留菜单开关状态打架；③受控输入清空用原生 value setter+input 事件（Playwright fill('') 在 React 受控框可能不触发 onChange）；④IAB 页签跨会话残留——走查前先看 tabs.list 清理。**下一步：小刀（任务路径分析+任务检查器）→PDF 对比余项（bar 图面标注/编制说明注/里程碑旗标/签证台账）→观察项（AOA 长计划画布宽缩放/适配）。详见 releases/v4.135.0.md。
- **最新发布：v4.134.0（2026-09-07）「进度计划刀D3：Excel 导入导出——上报口径往返」**——绑定面 **588→590**（gen_bindings 重生成；差距清单 #2，官方桌面客户端「Excel 导入/导出」项；Go excelize 复用成本板块依赖，格式层不入引擎无镜像负担）：**internal/schedule/xlsx.go**——ExportXlsx（上报 7 列 序号/WBS/任务名称/工期(天)/开始/完成/前置；CPM fail-closed 循环依赖不出表；日期 WdToDate 按日历换算；分组行跨连续叶任务 min ES/max EF；前置列 Project 式 `2FS+3` 引用）+ ImportXlsx（**表头别名归一识别**（去空格/括号/「天」字转小写精确匹配；任务名称/工作名称/工作内容…、工期/持续时间…、开始/开工日期/竣工日期…、前置/紧前工作/逻辑关系…、序号/标识号、wbs/编码/大纲；须含名称列+工期/开始/完成/WBS 任一）、**层级**=存在「X.」子 WBS 的行=分组（无 WBS 列时空工期行=分组；两级大纲收敛）、工期文本提取（`5 个工作日`→5）、前置引用 `2`/`2FS`/`2SS+3`/`2fs-1`（`(?i)` 大小写不敏感+归一大写）按序号回链、跳未知序号与涉及分组行引用、同键去重、里程碑=0 工期叶任务、**开始/完成列不进模型**（仅取最早开始作开工日）排程交回 CPM 重算与 mspdi 同律、Validate fail-closed 收口）。**绑定** GaeaScheduleExportXlsx（计划 JSON→xlsx base64）/GaeaScheduleImportXlsx（base64→计划 JSON）走 office 门面；前端 bridge AppBindings+名称映射、spaceBindings 分面 +2 work（**数量锁 301→303**，vitest 钉死）、mock/office.ts 直通桩（浏览器 dev 用 JSON↔base64 模拟往返，诚实可走查）、schedule/api.ts exportScheduleXlsx/importScheduleXlsx（分块 btoa/atob 防大文件栈溢出）、SchedulePage「导入 Excel/导出 Excel」按钮（FileExcelOutlined+importMsg 反馈）。go +7 用例（xlsx_test：往返一致/别名/引用变体含未知序号跳过/WBS 深度/无 WBS 分组/循环依赖 fail-closed/未知表头）。go test 全量绿、drift PASS@590、vitest 2301、tsc -b/eslint 0、版本四处 4.134.0、build 1m4。走查：导出回执+api 直连往返 20 行保名。**坑：①`gen_bindings -names` 输出纯名单到 stdout（漂移脚本据此对照），bindingNames.ts 的 TS 壳要自己包（别把名单直接重定向覆盖文件——我踩过，靠 git show HEAD 恢复格式）；②bash echo 中文注释进文件会编码花掉，TS 文件内容一律 python io.open 写；③vitest 必须在 frontend/ 下跑——从仓库根跑会丢 vitest.config 的 jsdom 环境（bridge.test 全挂 window is not defined 假警报）。**下一步：刀C 余项（undo-redo/右键菜单/任务筛选/进度编辑入口）→小刀（任务路径分析+任务检查器）→PDF 对比余项（bar 图面标注/编制说明注/里程碑旗标/签证台账）。详见 releases/v4.134.0.md。
- **最新发布：v4.133.0（2026-09-07）「进度计划刀D2：网络图上报件 + AOA 逆推锚点修正」**——绑定面 **588** 零变更（AOA 不入 Go 引擎）：**先修既有引擎缺陷**（图面照出：?mock=demo 无 deadline 却 min(ls)=−111）——aoa.ts 的 total（计算工期）在正推**之前**取事件注册初始 es（=单任务工期），长链计划下远小于真实 Tc（办公楼样例 30 vs 141）→ 逆推整体平移假负时差 + **刀G G3 的 AOA 侧 planFinish 锚点从未真实生效**（Tp 只与错误 total 比较）；修法=total 移到正推后；关键判定（相对比较）/ES/波形不受影响；+3 回归用例（长链 ls≥0 且 T.es=ls=Tc、planFinish<Tc 时 S.ls=Tp−Tc 且关键集含关键虚工作、planFinish≥Tc 不收紧）。**networkExport.ts 双构建器**：buildAoaExportSvg=时标网络+**工程标尺四行**（工程日步长 5/10/20 自适应+总工期红刻度/月 2026.9/日/星期；月界竖线贯通；日宽 36~110px/天按计划长度自适应防长计划爆画布；事件圈+上 ES/下 LS；箭线复用刀H edgeSegs/波形/过桥）；buildPdmExportSvg=拓扑分层+六格盒（TF=0 红字/绑定红链 isLinkBinding 同构副本/虚拟 S/T/无时标）；ganttExport 抽共享导出件（EXP_COLORS/EXP_FONT/EXP_MARGIN/EXP_TITLE_H/esc/fitText/exportTitleSvg/exportSignSvg）；ExportDialog 图面类型 Segmented（横道/双代号/单代号，默认跟随当前视图）。vitest 2291→2301（+10）、go 绿、drift PASS@588。走查：三图面 PNG/PDF 真实管线回执；AOA 图面目检（标尺日号跳周末/月标签/min(ls)=0/9 波形全在虚工作）；PDM 目检（红框关键链/红箭线 12 条/六格）。**坑复证：bash heredoc 传输会吃一层反斜杠——模板字面量里的全角空格必须写成 6 字符 u3000 转义（eslint no-irregular-whitespace 默认放行普通字符串但不放行模板字面量），python 批量替换用 chr(92)+'u3000' 构造最稳**。下一步：刀D3 Excel 导入导出（Go binding 588 会变）→刀C 余项→小刀（任务路径分析+任务检查器）。详见 releases/v4.133.0.md。
- **最新发布：v4.132.0（2026-09-07）「进度计划刀D1：图面导出——横道图上报件 PNG/PDF/打印」**——绑定面 **588** 零变更（纯前端；差距清单第一项 docs/gaea-schedule-gap-vs-project-2026-09.md，对标两份真实上报 PDF+MS Project 官方功能清单）：**ganttExport.ts 纯函数图面构建器**（发布物独立于工作台交互态，与 GanttView 同几何口径独立实现——标题带图名/编制单位/编制日期、上报 6 列 EXP_COLS 表格（前置 Project 式 2SS+5 引用、分组▲汇总）、自然日双行时标+非工作日底纹、关键红/手动灰/汇总黑/里程碑菱形/总时差尾、四型搭接依赖线（复用 ganttLinkPath）、目标竣工紫虚线、今日绿线、图例 6 项+图签三格（空值留白=签章位）；fitText CJK 全宽/半角 0.55 截断、XML 转义、循环依赖 fail-closed）；**imgPdf.ts 最小 PDF 封装**（单页 PDF 1.4+JPEG DCTDecode 直嵌零第三方依赖，MediaBox 72/96，xref 偏移逐字节对齐）；exportArtifact.ts DOM 管线（2 倍超采样 canvas→PNG/JPEG→PDF/隐藏 iframe 打印）；exportMeta.ts 签署字段持久化（chatPrefs 同范式）；SchedulePage「导出图面」弹窗三出口；ganttCols +GANTT_REPORT_KEYS/colsByKeys。vitest 2274→2291（+18；**用例抓出 imgPdf 初稿偏移表错位+条目数不一致**）、go test 绿、drift PASS@588。走查：真浏览器 PNG/PDF 导出回执+签署字段落盘复读；页面内同源 Vite 模块构建图面目检（20 行 14 条 12 关键）。**新坑：eslint no-irregular-whitespace 默认 skipStrings 但模板字面量仍查——全角空格写 　 转义；图面调色板按「图表数据」入 hex-exempt 整文件名单**。下一步：刀D2 网络图上报件（AOA/PDM 节点是 HTML 覆盖层不能克隆现场 SVG 须独立构建器+月/日/星期标尺）→刀D3 Excel 导入导出（588 会变）→刀C 余项→小刀（任务路径分析+任务检查器）。详见 releases/v4.132.0.md。
- **最新发布：v4.131.0（2026-09-07）「进度计划刀C：工作台双栏——表格｜画布可拖分栏 + 列显隐 + AI 栏折叠入口」**——绑定面 **588** 零变更（纯前端；用户点名 14 列表格挤画布/时间刻度竖排/AI 折叠）：横道重构「表格窗格｜分隔条｜画布窗格」真双栏（分隔条拖动收纳表格最小=行号+名称 194px、双击复位、纵滚同步=画布 scroll→表格体 translateY 直改 DOM 零重渲；**ganttCols.ts 纯函数** visibleCols/visibleLeftW/clampTableW/sanitizeHide+chatPrefs 扩展 ganttTableW/ganttHide 持久化）；列显隐菜单 12 列单列可藏（行号/名称固定）；**时间刻度竖排修复**=.sched-day-cell 补 nowrap+overflow hidden（dayW=8 两位日期折行元凶）+<10px 档只画格不写数；AI 栏折叠第二入口进栏头（i18n 三语 +1 键 schedChat.collapseTip）。vitest 2262→2274（+12；**用例抓出 visibleCols 初版把固定列也滤掉的真 bug**）、go test 绿、drift PASS@588。走查：拖窄 300→604 落盘+重挂载恢复/双击复位/隐藏「最迟开始」宽 838/全藏列钳 194/dayW=8 全格无字/AI 折叠展开往返。**新坑：ContextView+FilePreviewModal 两文件合跑互踩（28 假失败），各自单跑绿——既有环境 flaky 非改动引入**。刀C 余项（大纲折叠/undo-redo/进度编辑入口/右键菜单）与刀D 维持排队。详见 releases/v4.131.0.md。
- **最新发布：v4.130.0（2026-09-07）「进度计划基本功刀H：图例语言补全——标注归位 + 波形线 + 过桥法」**——绑定面 **588** 零变更（纯前端；审计渲染层三项缺陷 G2/G4/G1 销项）：**G2**=edgeSegs（edgeGeom 迁 aoaLayout 纯函数+段序列化）四类几何统一「名称在箭线上、工期在下」（波形时标注居中实体段）；**G4**=AOA auto 改**真时标轴**（x=最早时间×列宽 110px/天，时距空洞成比例；旧版按 es 排名分列跨边不可比）+水平段超出实体工期终点的尾段画波形（虚工作整段波形；手动布局 x 与时间解耦不画波形）+图例补「自由时差（波形线）/标注口径」两项；**G1**=findBridgeArcs 竖段垂直穿越他边横段处画半圆过桥（下行 sweep=1/上行 0 统一右凸；距段端<R+1、桥距<2R+4 护栏；端点相触不算），**PdmView 连线改段序列共用**并首次拥有组件测试。vitest 2241→2262（+21）、go test 绿（零改动）、drift PASS@588。走查（?mock=demo）：双代号 35 箭线 10 条波形**全非关键**/2 处过桥/20 组标注 0 错/27 节点真时标全对；单代号 2 处过桥/红搭接 7 条贯通。**新坑：图例内嵌小 svg 的 d 同样含 q/A——波形/过桥 DOM 断言必须按 path.sched-aoa-link 等连线类名取，querySelector('svg') 会先抓到图例图标；PDM 几何横竖段都在节点行中线上，普通两行项目只端点相触，过桥用例须 ≥3 行+跨列长横线构造**。下一步：刀C 交互包→刀D 周/月刻度+打印导图（AOA 真时标后长计划画布变宽，缩放/适配与刀D 一并评估）。详见 releases/v4.130.0.md。
- **最新发布：v4.129.0（2026-09-07）「进度计划基本功刀G：引擎诚实化」**——绑定面 **588 零变更**（TS/Go 镜像）：**E1**=freeFloatPart SS/SF 两型漏减前置 ES 锚点（LAG 应为 ES_j−ES_i−lag / EF_j−ES_i−lag），自由时差虚高可违反「TF=0⇒FF=0」定理——增 esFrom 参数并导出，四型公式单测钉死（实证 管道铺设 FF 11→0、沥青面层 2→0）；**G3**=目标竣工进逆推（ComputeCpmPlan/PlanFinish=DeadlineWorkdays 换算）：计划工期<Tc 时绑定链负时差、关键工作=TF 最小集，**minTF 仅在真正收紧时启用**（manual 驱动总工期时非手动链无 TF=0，维持经典口径防误标关键——旧用例钉死）；接线 SchedulePage/buildAoa（AOA 事件逆推同步收紧、箭线关键=浮时最小）/setBaseline/gschedSummary/mock analyze/Project.Analyze。vitest 2230→2241（+11）、go test ./... 绿（+4）。**坑：vitest 与 go test 并发跑必 flaky（31 假失败），门禁必须分开跑**。走查=雨污例（?mock=demo + __mockScheduleFile.save 钩子注入）FF 归零 + deadline 收紧绑定链 TF=−2/状态栏超 2 天。下一步：刀H（G2 AOA 标注颠倒/G4 时标波形线/G1 过桥法）→刀C 交互包→刀D 周/月刻度+打印导图。详见 releases/v4.129.0.md。
- **最新发布：v4.128.0（2026-09-06）「进度计划基本功刀F：单代号网络图合规整改」**——绑定面 **588 零变更**（纯前端；用户三连问实锤「连成一条直线/没分支/看不出关键线路」）。根因=PdmView 误用 layerByTime（列=最早时间），开工日互异的并行工作被拉成一条直线。整改：①**拓扑分层** layerByTopology 纯函数（列=最长路径深度不动点迭代+三轮重心松弛；并行分支同列并列；layerByTime 原样保留供 AOA 时标）；②多起点/多终点增画**虚拟起点 S/完成 T** 虚线框+虚箭线（JGJ/T 121 口径）；③**关键线路贯通**=两端关键且搭接为绑定约束时红色加粗（isLinkBinding：FS t.es===f.ef+lag/SS/FF/SF 四型，替换旧「仅 FS+lag0」漏标口径）；④节点行号角标与横道「3FS+2」引用互认。vitest 2224→2230（+6 layout 拓扑用例）；?mock=demo 走查 16 节点 2 行/13 层级/红搭接 12 条贯通/S+T 在位/自动适配 30% 一屏。**坑：走查截图出现旧帧（合成器滞留前视图画面）——DOM 断言为权威，切视图往返+等待后再截**。下一步：刀C 交互包/刀D 周/月刻度+打印导图。详见 releases/v4.128.0.md。
- **最新发布：v4.127.0（2026-09-06）「进度计划基本功刀A+E+B：呈现层合规整改」**——绑定面 **588 零变更**（纯前端，引擎六时参/四搭接/日历本就完整，坏在呈现）：**刀A**=AoaView edgeGeom 重写正交路由（前进 H-V-H 直角拐/同列竖直/后退下方通道；平行边错位 10px；标注随水平段归位——JGJ/T 121 禁斜线口径，走查 39 条箭线 0 斜线）；**刀E**=分组六色 16~18% 淡底+左色条（饱和实底退役，黑汇总条重见）、次级文字统一切 on-surface-variant（**根修：暗夜主题 outline 竟是 rgba(45,212,191,0.15) 15% 透明青——「日期看不清」元凶**）、坐标纸降强度 --sched-grid、图例/统计牌 sticky 吸顶、单代号缩放/适配全图（自动适配一次+手动 0.2~2 钳位；hooks 纪律：useEffect 必须在 early return 前）；**刀B**=横道 14 列（+LS/LF/TF/FF/后续；TF=0 红字；手动任务时差诚实显 —）+前置/后续 Project 式「3SS+5,4FS」引用文本（ganttLinks.ts fmtLinkRefs 纯函数）+依赖线按 FS/SS/FF/SF 四型取锚点（ganttLinkPath，含反向/短桩绕行；走查 18 条 0 斜线）。vitest 2216→2224（+8）；?mock=demo 走查 DOM 断言+三视图截图复核。**新坑：vite dev server 会静默退出先 curl 探活；antd Segmented radio 隐藏不可 role 点击要 DOM dispatch；首分组行类名是 c1 非 c0（gc 先自增）。下一步：刀C 交互包（列宽拖拽/大纲折叠/undo-redo/进度编辑入口/右键菜单）、刀D（周/月刻度+打印导图）；已知取舍=14 列 LEFT_W≈904px 左栏展开时图表窄（横向滚动+折叠可用）、单代号一行布局以适配缩放缓解折行观察后拍板。**详见 releases/v4.127.0.md。
- **最新发布：v4.126.0（2026-09-06）「本地模型使用：切换器模型粒度化 + 换模预估按目标模型全覆盖」**——绑定面 **588 零变更**（GaeaModelSwitchEstimate 仅扩展签名 engineID→engineID, model，方法数不变）：**刀1**=GaeaModels 按引擎展开全部 Kind=llm 模型（本地组置前/ModelInfo 增 local·status·label omitempty/空清单回退 "(默认)"/附带修 nil-client 空指针），GaeaSetModel("engine/model") 先 SetDefaultModel 再 SetActiveEngine（"(默认)" 只切引擎、清单外 fail-closed 报错防静默回落），前端 ModelSwitcher 分组+「本地」标+运行中徽标+展示名优先；**刀2 预估诚实化**（T5-3c 旧账：非 herdsman 恒 hot+herdsman 只查 DefaultModel）=herdsman 按目标模型查目录、ollama 原生 /api/ps+/api/tags（:latest 归一；ps 挂 tags 活保守 cold；双挂 unknown）、modelhub 复用 ModelHubModelLoaded；未加载 cold 不假报秒数（0→前端「需要一些时间」）。i18n 三语 +2 键；vitest 2214→2216（+2）；Go 全量 0 FAIL；?mock=1 走查 DOM 六项全过（坑：antd confirm 关闭后 DOM 残留 wrap display:none，查存在性会误报打开）。**下一步：真机池=本机 Ollama/Studio 实测预估与精确切换端到端；拍板池三文档等拍板。**详见 releases/v4.126.0.md。
- **最新发布：v4.125.0（2026-09-06）「联动候选：摘要卡设为当前计划 + 拍板池三设计文档落档」**——绑定面 **588 零变更**：①办公摘要卡非当前计划文件新增「设为当前计划」一键（normalizeProject 复制走既有 ScheduleSave fail-closed 通道→notifyScheduleFileChanged 即时回读；循环依赖计划不提供入口——Go Save 必拒收宁不显示；i18n +4 键）；②**拍板池三项设计文档落档**（并行子代理调研，拍板材料齐备）：diff 确认卡=混合分级确认（project 整量前置强制/ops 审批卡升级 diff 卡/后置回滚入口，零绑定可行已核实——ToolDispatch 先于闸门前端已持 args，最大风险=simulateOps TS 镜像与 Go ApplyOps 漂移，4 刀 8 拍板项）+ 双工期口径=durationUnit 'wd'|'cd' 单位标记（28cd≈20 工作日跨度 ceil 吸附，v1 cd 仅 FS 搭接收窄，4 刀 9 拍板项，mspdi elapsed 序列化绑真机）+ 多工程管理=每文件一工程+.gaea/schedule/index.json 索引+Go 侧当前指针（刀1 绑定 588→590 或零新增备选，agent 缺省 path 跟随指针，8 拍板项）。vitest 2213→2214（+1）。**下一步：三份文档等用户拍板即可进实施刀序；资源成本刀4 绑真机池。**详见 releases/v4.125.0.md。
- **最新发布：v4.124.0（2026-09-06）「资源成本刀2+刀3：agent 通道 + 板块 UI」**——绑定面 **588 零变更**（并行双线子代理实施：Go 侧/前端侧文件面零交叠，主代理合并审读+?mock=1 走查后同版发布）：**刀2 agent 通道**=ops 扩枚举 upsert_resource/patch_resource（指针三态）/remove_resource（级联删分配）/set_assignments（整体替换，set_links 同范式）+patch_task.fixedCost，op 层 fail-closed（分组行禁挂/悬空/负值/集内重复）；**apply 回执强制带 totalCost**（+taskCosts，证据卡追加总成本——「成功≠正确」延伸）；get 增 resources/assignments/costs；analyze 增成本叙事（Top5/byResource/「已分配未定价」finding=AI 建议层）；compact 三条 compactDesc 同步；技能「资源与成本」分节，**锚点锁 23→31**。**刀3 板块 UI**=资源工作表弹层（ResourcePanel.tsx：单位显式标注 元/工日·元/单位、cost 类型隐藏费率并联动清理、删除级联）；甘特「成本」列（叶明细+分组滚动求和，无数据留空）；任务行资源 Popover（勾选建分配+三类字段+整体替换，分组行不渲染入口）；状态栏总成本段（无数据不显示）；store 容错（负值丢弃回落 undefined 与 normalize 同律/分组行 no-op）；**removeTask 级联删 assignments**（防悬空被 Validate 拒收致自动保存失败——刀2 Validate 与前端 store 的接缝缺陷，UI 线先行发现并修复）。vitest 2194→2213（+19）、Go +18。**?mock=1 全链走查贯通：录入力工 300 元/工日→勾选建分配→成本列 3000→资源合计/状态栏 ¥3000→已保存落盘**。新 mock 场景 `?mock=calm`（无 running 子代理，消除自动置前走查噪声）。欠账：刀4 mspdi（绑真机池不真机不发）；资源使用视图/超载检测/基线成本快照候选。详见 releases/v4.124.0.md。
- **最新发布：v4.123.0（2026-09-06）「AOA 手动布局刀1：锚点化 + 拖拽布点 + 双模式开关」**——绑定面 **588 零变更**（按 docs/gaea-schedule-aoa-manual-layout-design-2026-09.md §6 推荐拍板项实施：混合锚定/pins 内嵌/mode 不入文件/半格吸附/agent 不暴露布局）。**布局=展示层状态，buildAoa 引擎口径零变更**：①锚点键 AoaNode.anchor=事件业务身份（S/T/成员键 end:/start: 字典序最小者），跨拓扑变更稳定（稳定性 5 例钉死）；②aoaLayout.ts 纯函数 snapPt 半格吸附（55×37）/normalizeAoaLayout 结构归一/applyPins 混合共存/prunePins 提交剪枝；③持久化 gsched.json 内嵌 aoaLayout.pins（TS↔Go 镜像，Go 结构校验 pass-through 坐标 0~100000+键数 5000 上限；**维持无 aoa.go**——镜像铁律约束引擎口径，布局数据不属引擎口径）；④AoaView 双模式 Segmented+重置布局+拖拽布点（对齐刀9 纪律：未移动不提交/Esc 取消/自动拖拽=转手动；提交时剪枝失配键）。vitest 2179→2194（+15）。**zustand 坑再入册：selector 里 `?? {}` 每次新引用→getSnapshot 不缓存→无限重渲，派生放渲染体**；setState updater 内禁副作用（commitPin 放渲染期外）。vitest flaky 新增 CostProjectsView（负载型，单跑复跑绿）。欠账：?mock=1 拖拽走查+真机手感入真机池；刀2 对齐辅助线/一键整理观察后拍板。详见 releases/v4.123.0.md。
- **最新发布：v4.122.0（2026-09-06）「资源成本刀1：模型 + 成本 rollup 引擎 + 持久化」**——绑定面 **588 零变更**（按 docs/gaea-schedule-resource-cost-design-2026-09.md §5 推荐拍板项实施：元/工日口径、fixed-units 简化公式、材料固定总量、分组行禁 fixedCost/分配、EVM 不做）：①模型 types.ts/types.go 双侧同构——三类资源（work/material/cost）+ SchedAssignment（(taskId,resourceId) 唯一）+ Task.fixedCost；SchedProject.resources/assignments 全 optional **旧文件零迁移可读**；②cost.ts/cost.go rollup 镜像（work=工期×units×费率+每次使用；material=总量×单价+每次使用；cost=金额；rows 只含叶任务；CPM 循环→ok=false fail-closed；悬空分配跳过；金额到分舍入 math.Round 正值与 Math.round 同律）；③两道闸分工同现状——前端 normalize 补空数组+坏形条目逐条丢弃，Go Validate fail-closed（资源 id 唯一/类型合法/数值非负有限、分配引用存在+对唯一+**分组行禁挂分配**、FixedCost 非负）。测试镜像纪律：TS +13、Go +14（含 Save/Load roundtrip 保资源）。vitest 2166→2179。后续：刀2 agent 通道→刀3 板块 UI→刀4 mspdi（绑真机池）；AOA 手动布局刀1 顺序实施中（共用 SchedProject schema 不并行）。详见 releases/v4.122.0.md。
- **最新发布：v4.121.0（2026-09-06）「进度计划刀12：办公联动三件套——计划摘要卡 · 板块互跳 · AI 周报一键」**——绑定面 **588 零变更**（方向拍板：办公×进度**不并板只联动**——数据/AI 层刀4~11 已是「单资产、双入口、事件互通」终态，本刀只补交互层）：①办公文件预览 `.gsched.json` 出**计划摘要卡**（ScheduleFileCard：总工期/工作/关键/搭接/里程碑 chips+开竣工日期+基线漂移+倒排校核+循环依赖告警；原始 JSON 开关保留文本视图；解析失败回落原文本视图宁回落勿误报）；②**板块互跳**——摘要卡「在进度计划中打开」/ 进度板块页头 ToolOutlined「办公侧查看」反向按钮（`usePreviewStore.openFilePreview` store 预置+navigate，office 未挂载也落地）；跳转/周报**仅对当前计划文件**提供（板块绑定当前计划，其余 .gsched.json 只读摘要防误导）；③「AI 进度周报」一键发**共享工作线程**（运行中 Steer 插话/空闲 send 同构落气泡 Submit；审批挂起/内核未就绪禁用）。纯函数 schedule/gschedSummary.ts（isScheduleFilePath+parseSchedSummary，失败返回 null）与 Go DefaultRelPath 镜像常量 SCHEDULE_FILE_PATH。i18n 三语 +27 键（schedCard.*）。vitest 2151→2166（+15：gschedSummary 8+卡片 7，首跑 1 例已知负载 flaky 复跑全绿）。**联动坑：gaea/icons.ts 无 ChevronUp（ArrowUp 有）——icon 集先查再引，undefined 组件运行时才炸**。拍板池：diff 确认卡/双工期口径/多工程；非拍板余：双代号手动布局/资源成本（需设计方案）；联动候选：完成率口径/周报模板化/另存为当前计划；真机池：真机端到端+联动跳转手感。详见 releases/v4.121.0.md。
- **最新发布：v4.120.0（2026-09-06）「进度计划刀11：左栏细节完善——拖宽分隔条 · 偏好持久化 · 快捷指令条」**——绑定面 **588 零变更**：刀10 双栏细节收口——①宽度拖拽分隔条（hover 显色、320~680 钳位、松手持久化、双击复位 420）；②偏好持久化 schedule/chatPrefs.ts（localStorage gaea.schedule.chatPrefs，字段级容错+可注入 storage，+5 例）；③新会话 Popconfirm 二次确认（文案顺带教育线程共享语义）；④常用指令快捷条 4 chip（点击直发、运行中禁用，对应 schedule-edit 三大工作流）；⑤头部副标题「与办公板块共享同一条工作线程」。i18n 三语 +6 键。vitest 2151/2151（+5）。**走查环境坑：合成事件拖宽期间物理鼠标真实 mousemove 混入致读数漂移——真实用户无此噪声，非产品缺陷**。拍板池：diff 确认卡/双工期口径/多工程；非拍板余：双代号手动布局/资源成本（需先出设计方案）；真机池：真机端到端+手感。详见 releases/v4.120.0.md。
- **最新发布：v4.119.0（2026-09-06）「进度计划刀10：左 AI 对话 · 右工作台——双栏版面 · 共享工作线程」**——绑定面 **588 零变更**（用户点名版面）：进度计划板块改双栏，左 420px AI 对话栏（可折叠）+右工作台零删减；**与办公 GaeaApp 共享同一条工作线程**（gaea/lib/store 模块单例，走查互认实证：左栏发的消息办公对话区可见）；**架构关键=store 事件绑定提升模块级恰好一次（ensureEventsBound）**——原 useController 每挂载订阅一次，keepAlive 双挂载会双订阅致流式增量重复（气泡翻倍），首个宿主绑定/卸载不退订（隐藏期事件继续入库回板块即热态），看门狗保持每宿主；store.host.test 钉死双宿主单订阅；ScheduleChatPane=Transcript+Composer+ApprovalModal+AskCard 装配+schedule_apply done→notifyScheduleFileChanged；i18n 三语 +4 键。vitest 2146/2146（+1）。欠账：对话栏宽度拖拽/双代号手动布局/资源成本（需设计方案）；拍板池：diff 确认卡/双工期口径/多工程；真机池：真机端到端+手感。详见 releases/v4.119.0.md。
- **最新发布：v4.118.0（2026-09-06）「进度计划刀9：横道拖拽调排程——拖移即定位锁定 · 右缘缩放改工期 · 直接操作闭环」**——绑定面 **588 零变更**：语义钉死=拖移 auto 任务转手动锁定到落点（Project 约束同源；auto 位置由搭接决定，拖完会被 CPM 拉回，转手动是唯一诚实做法）、manual 拖移=更新 manualStart、右缘缩放=改工期（里程碑可拖不可缩、分组行不可拖）；落点按最近工作日吸附（drag.ts workdayOffsets/nearestWd 等距取左）；Esc 取消、未移动=无操作、循环依赖（CPM 不过）整体禁拖；拖拽中条形虚线描边+浮动提示（「第 5 → 8 工作日（转手动）」/「工期 15 → 16 天」）；提交走 updateTask→防抖保存→agent 可见。GanttView 首次拥有组件测试（fireEvent 鼠标序列 5 例）。vitest 2145/2145（+11）。**新走查坑：overflow:auto 容器边缘元素被裁剪致 elementFromPoint 命中落空**——缩小刻度让目标离开边缘即可。拍板池余：diff 确认卡/双工期口径/多工程；非拍板余：双代号手动布局/资源成本；真机池：真机端到端+拖拽手感（原生手势需真人）。详见 releases/v4.118.0.md。
- **最新发布：v4.117.0（2026-09-06）「进度计划刀8：倒排工期——目标竣工约束 · 引擎可行性裁决 · 横道竣工线」**——绑定面 **588 零变更**：对齐 Project「必须完成期限」，立场=引擎裁决不自动改排程（自动锁死被拒：违背 manual 语义；压缩方案 AI 建议用户拍板）。deadline 存日历日期（开工日调整后语义稳定）；targetWorkdays=[开工日,竣工日]工作日计数（非工作日竣工回落；早于开工=0 必不可达）；TS deadline.ts+calendar.deadlineWorkdays ↔ Go deadline.go+DeadlineWorkdays 镜像；ops set_meta{deadline} 指针三态；三工具接线（get deadline/apply 回执 deadlineCheck/analyze 倒排发现——不可达报超期量+压缩对象、富余≤2 提示触线、充足不出发现）；UI=页头目标竣工输入+状态栏倒排段（超期红/富余绿/压线达成）+横道紫虚线竣工线（画竣工日列末）；schedule-edit 第 3 步改写（锚点锁 20→23）。vitest 2135/2135（+9）、Go +9。拍板池余：diff 确认卡/双工期口径/多工程；非拍板：横道拖拽/资源成本；真机池：真机端到端+基线/倒排 UI 手感。详见 releases/v4.117.0.md。
- **最新发布：v4.116.0（2026-09-06）「进度计划刀7：基线对比——快照固排程 · 漂移量化 · 板块/agent 同口径」**——绑定面 **588 零变更**（基线随计划文件存取复用 GaeaScheduleLoad/Save）：对齐 Project「设置基线」，TS baseline.ts ↔ Go internal/schedule/baseline.go 镜像（快照 fail-closed：循环依赖/无叶任务拒绝；漂移对比=总工期漂移+推移/新增/移除+关键链进出，rows 只含有偏差行、移除行按 id 排序；单关键标记翻转也计推移）；ops 增量通道 set_baseline（缺 savedAt 拒绝——引擎纯函数不取时钟，工具层盖时间戳）/clear_baseline（无基线报错≠静默成功）；agent 三工具接线（get 基线元信息/apply 回执 baselineDrift 摘要/analyze baselineDrift 全量+checks「较基线」发现漂移≠0 才出现）；板块 UI（页头基线弹层保存/更新/清除+偏差行清单、横道图灰描边基线条垫行底+开关、状态栏基线段拖后红提前绿）；schedule-edit 技能「基线对比」节（锚点锁 17→20）。vitest 2126/2126（+11）、Go internal/schedule 21→32 测试函数+工具链 1 例。口径=工作日偏移（相对开工日，不受开工日调整影响）；mspdi 不携带基线（观察项）。拍板池余：diff 确认卡/双工期口径/多工程；真机池 +基线 UI 手感。详见 releases/v4.116.0.md。
- **最新发布：v4.115.0（2026-09-06）「进度计划刀6：推荐逻辑关系——auto_chain 确定性原语 · 无前置质检 · 报告模板」**——绑定面 **588 零变更**：ops auto_chain（仅为无前置叶任务按 WBS 顺序补 FS 串联；已有逻辑/分组/手动不动——手动忽略入边不作链源；无可补拒绝≠静默成功）；schedule_analyze 质检「N 个任务无前置搭接」发现（首叶/手动不计，文案指路 auto_chain→set_links；工具测试闭环：报 1→auto_chain→清零）；schedule-edit 技能增「推荐逻辑关系」三步工作流（auto_chain 缺省→工艺例外逐条覆写给理由可回滚）+「分析报告模板」四段式（进度/风险/盲点/建议，Asana Smart Status 口径），锚点锁 +4。Go schedule 33 例。拍板池余：diff 确认卡/双工期口径/多工程/真机端到端。详见 releases/v4.115.0.md。
- **最新发布：v4.114.0（2026-09-06）「进度计划刀5：对话即排程——schedule-edit 技能 · agent 写板块即回读 · 工具卡摘要」**——绑定面 **588 零变更**：schedule-edit 内联技能（编制/调整纪律总纲：JGJ/T 121-2015 先分解后编网、先读后写、回执核对「成功≠正确」、引擎裁决、压缩非关键不缩总工期要点破；+1 锚点锁测试）；App 工具事件喂入 schedule_apply done→notifyScheduleFileChanged()（板块即时回读，守卫宁守勿冲）；tools.ts 三工具卡摘要（总工期/关键/修改数，信封剥离兜底）。vitest 2115/2115（store.sync 全链用例：迁移→防抖保存→外部写回读）、Go 全量绿、drift PASS@588。真机端到端入真机池。详见 releases/v4.114.0.md。
- **最新发布：v4.113.0（2026-09-06）「进度计划刀4：AI 原生通道地基——计划文件化 · agent 工具三件套」**——绑定面 586→**588**（+GaeaScheduleLoad/Save，drift PASS@588，分类锁 301）：internal/schedule 新包（前端引擎忠实移植：CPM/日历/校验原子写/ops 增量操作集/叙事分析，28 例与 TS 测试互为镜像）；agent 工具 schedule_get/apply/analyze（apply 双通道 project/ops、快照+证据卡+confine、环 fail-closed 回传错误原文、回执强制带新总工期——**AI 产建议、引擎裁决**）；计划文件 进度计划/当前计划.gsched.json 板块与 agent 共享（水合 localStorage 迁移+防抖自动保存+focus/15s 回读；UI 保存无证据卡，证据卡只属 agent apply）；市场调研落档 docs/market-research-2026-09-06.md（15 原始稿；对话式调整安全空白 gaea 自定义标准；斑马 AI 11 项=analyze 需求清单；**勘误 JGJ/T 121-2015**）。刀5 拍板池：对话式调整 diff 确认闭环/整计划生成三确认/推荐逻辑关系/报告模板化/双工期口径。详见 releases/v4.113.0.md。
- **最新发布：v4.112.0（2026-09-06）「进度计划刀3：斑马 UI 对齐——六色分组横道 · 网络图统计牌 · 底部状态栏」**——绑定面 **586 零变更**（纯前端展示层）：横道图分组行六色饱和循环（橙/青/粉/黄/蓝/绿+▲组名+左色条）+行号列+时间轴改自然日列（非工作日底纹，CPM 仍按工作日）；单代号/双代号统计牌（红字大数字，走查实锤浮层遮节点→改挂图例行右端）；底部状态栏常驻（共 N 项工作·总工期 N 天）；?mock=1 三视图走查全过；**走查新坑：vite 同文件连续快编辑留中间版 transform 缓存（后半编辑丢失、reload 无效），touch 强制重转换即恢复**。附带补账：README 版本表补 v4.104~v4.111 八行、本文件顶部状态补八版一行总览（八版漏同步的发布纪律缺口）。vitest 2110/2110、drift PASS@586。详见 releases/v4.112.0.md。
- **v4.104.0~v4.111.0（2026-09-06，八版一行总览，细节见各自 releases/ 与 CHANGELOG）**：v4.104 MH1-MH3 modelhub 采样让位+默认关思考+UD 置顶（584）；v4.105 MH4 常驻预热+CU1 ComfyUI 预热+CU2 fp8 档位（+WarmComfyUI=585）；v4.106 xlsx 条件格式预览+HS-obs 销账（585）；v4.107 多维表看板视图（585）；v4.108 导图画布编辑 M2 自研 v1（585）；v4.108.1 ?mock=1 三链走查+mock 走查态升级（585）；v4.109 pptx 真编辑刀1 GaeaPptxApplyEdit（585→586）；v4.110 进度计划刀1 CPM+三视图+板块接线（586）；v4.111 进度计划刀2 日历/手动自动模式/WBS/mspdi XML/前锋线（586）。
...
- **最新发布：v4.101.0（2026-09-05）「三线并行：GenUI 围栏 Go 侧收口 · 画室
  ...
  逐字保留；做梦/compact 摘要/whisper 摘要三处剥离；压缩 prompt 围栏指引；
  面板 resync 去重改 spec 指纹；会话删除清理交互状态；第 5 项 resume 口径
  另行排期）；画室新增「模型目录」tab（HerdsmanModelCatalog 只读按能力分族，
  目录无档位/成本→未定价不伪装 0，创作语境高亮不做管理）；深检误报缓解
  （归一化+别名归一+置信度/原因三档只降不升零静默吞+忽略记忆可恢复）。
  Go 全量 0 FAIL（含他线在途如实注明）、vitest 254/2024、drift PASS（581
  口径）、模型目录 ?mock=1 走查 PASS。详见 releases/v4.101.0.md。
...
- **最新发布：v4.98.0（2026-09-05）「图像域 T0 契约落地：登记底座 · 素材库 ·
  ...
  + 模型中心目录只读视图；试点书封/绘梦登记、OCR/识图入口校验（行为不变）；
  T1 提前量（章节配图落盘登记 + 本章历史配图、绘梦「素材库」tab、角色剧照
  溯源）；T2 参考槽 v0（绘梦选角色→参考图图生图近似，ipadapter/pulid 诚实
  拒绝）；办公 U1 FORMAT_* 结构化错误码 + office-edit 内置技能；修登记闸
  测试污染（App.Startup 显式武装位）。Go 全量 0 FAIL、vitest 245/1898、
  drift PASS（581）、冒烟 200。真机验收待用户复验。详见 releases/v4.98.0.md。
...
- **最新发布：v4.93.0（2026-09-05）「失败子代理恢复入口 + diff 行内
  ...
  接线 ChangesDiff（变更面板/Git 面板共用）。vitest 231/1756、Go 全量
  0 FAIL、drift PASS（578）、?mock=1 DOM 走查通过。详见 releases/v4.93.0.md。
- **此前：v4.92.0（2026-09-05）「Mermaid strict 安全线成文 +
  MemoMarkdown 消毒层」**——3b 收口；绑定面 578 零变更：MemoMarkdown 接
  sanitizeHtml；SVG 外层再消毒实验证伪后有意不做（决策成文）。vitest
  229/1753、Go 全量 0 FAIL、drift PASS（578）、?mock=1 DOM 走查通过。
  详见 releases/v4.92.0.md。
- **此前：v4.91.0（2026-09-05）「CodeMirror 语法高亮编辑器」**——
...
- **此前：v4.89.0（2026-09-05）「成本费率 hover」**——调研回填中期
  ...
  ?mock=1 DOM 走查通过。详见 releases/v4.89.0.md。
- **此前：v4.88.0（2026-09-05）「/context 居中弹层 + 常驻剩余上下文%
  徽标」**——2.5e 收口；绑定面 578 零变更：ContextPill codex 式常驻徽标
  （点击开弹层）+ ContextModal 复用 ContextView；/context 命令改道弹层。
  vitest 227/1743、Go 全量 0 FAIL、drift PASS（578）、?mock=1 DOM 走查
  通过。详见 releases/v4.88.0.md。
- **此前：v4.87.0（2026-09-05）「统一 diff 渲染升级」**——2c 刀，
...
- **此前：v4.86.0（2026-09-05）「Git 面板最小集」**——2b 刀，D3 采纳
  ...
  ?mock=1 DOM 走查四项通过。详见 releases/v4.86.0.md。
- **此前：v4.85.0（2026-09-05）「本轮文件三态折叠：写入/编辑/读取
  独立成层 + 类型筛选」**——2a 刀，绑定面 571 零变更：ChangesPanel 三层
  独立折叠（读取层轻量行默认收起）+ 类型筛选 chips；diff/回滚保真。
  vitest 224/1720、Go 全量 0 FAIL、drift PASS（571）、?mock=1 DOM 走查
  通过。详见 releases/v4.85.0.md。
- **此前：v4.84.0（2026-09-05）「HTML 沙箱预览 · 外链协议分流」**——
  1c 刀，阶段一收口；绑定面 571 零变更：.html kind=html 沙箱 iframe
  （sandbox 无同源 + csp default-src 'none' 双保险 + 标注条）；
  classifyExternalLink 渲染链接协议分流接线三处。vitest 224/1712、Go 全量
  0 FAIL、drift PASS（571）、?mock=1 DOM 走查通过。详见 releases/v4.84.0.md。
- **此前：v4.83.0（2026-09-05）「工具结果图片缩略卡：官方 patch 口径
  token 估算」**——2.5b 后半刀，阶段二.5 收口；绑定面 571 零变更：官方
  口径 ⌈w/28⌉×⌈h/28⌉（官方例 1000×1000→1296 单测锚定）；缩略卡成对显示
  「原始尺寸→缩放尺寸」+「≈N tok · 标准档」；缺失/不可解码诚实降级；
  fold 修 stats.Images 死字段。vitest 224/1700、Go 全量 0 FAIL、drift
  PASS（571）、?mock=1 DOM 走查通过。详见 releases/v4.83.0.md。
- **此前：v4.82.0（2026-09-05）「上下文趋势跳转浏览器：brief 行锚点 ·
  偏好持久化 · 设置中心默认」**——2.5d 刀，绑定面 571 零变更：Go fold 透出
  briefUserSeq/briefRespSeq（工具交换锚结果节点）；趋势卡 brief 行点击→浏览器
  组展开+滚动+高亮 3s；粒度/模式/排序偏好持久化+设置中心「上下文页偏好」卡。
  vitest 224/1698、Go 全量 0 FAIL、drift PASS（571）、?mock=1 DOM 走查通过
  （截图通道故障如实记录）。详见 releases/v4.82.0.md。
- **此前：v4.81.0（2026-09-05）「文件活动行级增量」**——±徽标/操作日志展开/
  详情跳转；1b 勘误销账。详见 releases/v4.81.0.md。
- **此前：v4.80.0（2026-09-04）「上下文工具结果深读」**——懒加载完整调用/
  来源 chip/分类内排序。详见 releases/v4.80.0.md。
- **此前：v4.79.0（2026-09-04）「上下文对比上一步」**——逐类 signed delta/
  跨压缩近似。详见 releases/v4.79.0.md。
- **此前：v4.78.0（2026-09-04）「任务强制终止收口」**——进程树击杀/两击确认。
  详见 releases/v4.78.0.md。
- **此前：v4.77.0（2026-09-04）「任务页自动激活补全 · 强制终止」**
  ——纯前端、绑定面 561 零变更：后台任务事件自动激活任务页；自动激活统一
  openTasksAuto（≥1240px 宽屏展开、窄屏不强制、设置开关可关）；运行中任务
  按钮明确「强制终止」（queued=取消 / stopping=停止中）。vitest 223/1681、
  tsc/eslint 0、冒烟通过。详见 releases/v4.77.0.md。
- **此前：v4.76.0（2026-09-04）「任务面板修正」**——「子代理」分组头点击
  整体折叠/展开整棵树（点卡片也可折叠）；卡片圆角显式 8px；子树加树形线；
  打开对话收敛为卡片独立按钮（subagent.openThread 三语新增）。vitest
  223/1680。详见 releases/v4.76.0.md。
- **此前：v4.75.0（2026-09-04）「右侧任务面板卡片化」**——AgentTree 升级为
  独立子代理卡片：标题+状态胶囊+模型/工具/耗时/token 指标行，运行卡 glow
  描边+实时预览，完成卡展示回答摘要；视觉模型验收 92/100。vitest 223/1678。
  详见 releases/v4.75.0.md。
- **此前：v4.74.0（2026-09-04）「记忆界面技能重设计」**——记忆主区页改
  hero + 三统计小卡 + 快速添加 + 单张视图大卡（分段/搜索/内容同卡），
  TabButton 紧凑胶囊；视觉验收 9/10。vitest 223/1678。详见
  releases/v4.74.0.md。
- **此前：v4.73.0（2026-09-04）「记忆迁入主区标签页」**——删左侧「记忆」
  入口、ChatTabs 增「记忆」tab（上下文旁）、记忆界面卡片化、抽屉路由收敛。
  vitest 223/1678。详见 releases/v4.73.0.md。
- **此前：v4.72.0（2026-09-04）「删除概览标签页」**——ChatTabs 收敛
  对话/轨迹/上下文；删除 OverviewPanel 容器与命令面板「概览面板」条目；
  底层 StatsPanel 暂留未接线。vitest 223/1677。详见 releases/v4.72.0.md。
- **此前：v4.71.0（2026-09-04）「上下文页卡片化」**——8 统计项拆独立小卡
  （ctx-tile 2→4→8 列）、Token/耗时/会话三大仪表卡图标章、事件/文件/浏览器
  节点行 cardlet 化（ctx-row）；自研规划不硬抄参考图。vitest 224/1683。
  详见 releases/v4.71.0.md。
- **此前：v4.70.0（2026-09-04）「上下文页视觉重设计：数字层级 · 构成强调 ·
  仪表化」**——用户点名「重新设计」后 v4.69 交互精修静态不可见（承认），本轮
  改肉眼可见。纯前端、绑定面 561 零变更、Go 零改动；逐项对齐 dsh-context
  styles.css 实测数值：①数字层级放大（统计格 15/17px、当前上下文大数字+
  百分比 26px bold、环心 16px、各行 11.5px）；②当前上下文宽卡强调（分段条
  h-3→h-4 rounded-md、卡头 12.5px semibold、图例 11.5px）；③卡头「标题左+
  副注右」统一；④环形图 84→92px/stroke 10px；⑤v4.69 事件多选/趋势内联详情/
  99.89% 保留。vitest 224/1683、tsc -b/eslint 0、drift PASS（561）、冒烟通过。
  详见 releases/v4.70.0.md。
- **此前：v4.69.0（2026-09-04）「上下文页第三轮精修：图例联动 · 趋势卡内联详情 ·
  事件多选」**——继续对齐 dsh-context v0.41.3（用户实拍两图 + 源码复核）。纯前端、
  绑定面 561 零变更、Go 零改动：图例 chip hover 联动（悬停分类该段保持、其余段与
  空闲段 150ms 淡出）；趋势卡 master-detail 单元（详情内联卡内 + 默认选中最新
  请求）；事件筛选改 dsh 多选 chips；四卡头「标题+右副注」单行排版；Token 环心
  两位小数；死键 pickHint 三语删除。vitest 224/1683（+2）、tsc -b/eslint 0、
  drift PASS（561）、冒烟通过。详见 releases/v4.69.0.md。

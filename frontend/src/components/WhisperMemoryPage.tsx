  ...
  return (
    <div style={{ padding: 16, display: 'flex', flexDirection: 'column', gap: 12, height: '100%' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        <span style={{ fontSize: 13, fontWeight: 600, color: '#d4d4d8' }}>🧠 记忆管理</span>
        <Tag style={{ marginLeft: 'auto' }}>{facts.length} 条</Tag>
      </div>

      <Input prefix={<SearchOutlined />} placeholder="搜索记忆…" size="small"
        value={search} onChange={e => setSearch(e.target.value)}
        style={{ background: '#27272a', border: '1px solid #3f3f46', borderRadius: 8 }} />

      <div style={{ flex: 1, overflow: 'auto' }}>
        {filtered.length === 0 ? (
          <div style={{ textAlign: 'center', color: '#71717a', padding: 24, fontSize: 12 }}>
          ...
          filtered.map(f => (
            <div key={f.id} style={{
              ...
              borderLeft: f.tier === 'core' ? '3px solid #f59e0b' : '3px solid #3f3f46'
            }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                <span style={{ fontSize: 12, fontWeight: 500, color: '#d4d4d8', flex: 1 }}>{f.subject}</span>
                <span style={{ fontSize: 10, color: '#71717a' }}>{DOMAIN_LABELS[f.domain] || f.domain}</span>
                {f.tier === 'core' && <Tag color="gold" style={{ fontSize: 9, margin: 0 }}>核心</Tag>}
                <Button type="text" size="small" icon={<EyeOutlined />}
                  onClick={() => setSelectedFact(f)} style={{ color: '#a1a1aa' }} />
                {onDelete && (
                  <Button type="text" size="small" danger icon={<DeleteOutlined />}
                    onClick={() => { onDelete(f.id); message.success('已删除') }} />
                )}
              </div>
              <div style={{ fontSize: 11, color: '#71717a', marginTop: 4, lineHeight: 1.5 }}>
      ...
      <Modal title={selectedFact?.subject} open={!!selectedFact}
        ...
        {selectedFact && (
          <div style={{ fontSize: 13, lineHeight: 1.8, color: '#d4d4d8' }}>
            <p>{selectedFact.summary}</p>
            <div style={{ color: '#71717a', fontSize: 12, marginTop: 12 }}>

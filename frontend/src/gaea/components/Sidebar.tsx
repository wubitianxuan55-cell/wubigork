...
export function Sidebar({
  ...
  const renderSessionItem = (session: SessionMeta) => (
    <div
      className={`group flex items-start gap-1 rounded-lg py-1.5 pl-2.5 pr-1.5 mb-0.5 transition-colors duration-150 hover:bg-sidebar-hover ${
        session.current ? "bg-sidebar-active border-l-2 border-accent pl-[9px]" : "border-l-2 border-transparent"
      }`}
      key={session.path}
    >
      <button
        className="flex items-start gap-2.5 flex-1 min-w-0 bg-transparent border-0 text-inherit cursor-pointer py-0.5 text-left disabled:cursor-default"
      ...
      >
        <MessageSquare
          size={13}
          className={`shrink-0 mt-[3px] ${session.current ? "text-accent" : "text-fg-faint/70"}`}
        />
          ...
          {renameTarget === session.path ? (
            ...
            <>
              <span className="flex items-baseline gap-2 min-w-0">
                ...
                <span className="shrink-0 ml-auto text-fg-faint/60 font-mono text-[10px] tabular-nums">
                  {session.current ? t("history.current") : relativeTime(session.modTime)}
                </span>
              </span>
              {!session.current && session.preview && (
                <span className="flex-1 min-w-0 overflow-hidden text-ellipsis whitespace-nowrap text-fg-faint/55 text-[11px] leading-snug">
                  {session.preview}
                </span>
              )}
            </>
      ...
      {!session.current && (
        deleteConfirm === session.path ? (
          ...
          <button
            className="hidden group-hover:flex items-center justify-center w-5 h-5 shrink-0 mt-1 rounded-md bg-transparent border-0 text-fg-faint text-[13px] cursor-pointer hover:text-err hover:bg-bg-soft transition-colors"
            title="删除"
  ...
  return (
    <>
      ...
      >
        {/* 头部：logo + 名称 + 新建 / 折叠（Codex 风格紧凑栏） */}
        <div className={`flex items-center gap-2 px-1.5 pb-3 ${collapsed ? "flex-col gap-2 px-0 pb-3" : ""}`}>
          ...
          <div className={`flex items-center gap-1 ${collapsed ? "flex-col" : "ml-auto"}`}>
            <button
              className="inline-flex items-center justify-center w-7 h-7 border-0 rounded-md text-accent bg-accent/12 cursor-pointer transition-[color,background,transform] duration-[var(--dur-fast)] hover:bg-accent/22 active:scale-95 no-drag disabled:opacity-40 disabled:cursor-default"
        ...
        {!collapsed && (
          <section className="flex-1 min-h-0 flex flex-col">
            {/* 搜索框（Codex 风格：前置放大镜、柔和底色） */}
            <div className="relative mb-2">
              <Search size={13} className="absolute left-2.5 top-1/2 -translate-y-1/2 text-fg-faint/70 pointer-events-none" />
              <input
                className="w-full h-8 pl-8 pr-2.5 bg-bg-soft/70 border border-border-soft rounded-lg text-fg text-xs placeholder:text-fg-faint/60 outline-none focus:border-accent/50 transition-colors no-drag"
            ...
            <div className="flex items-center gap-2 px-1.5 pb-1.5">
              <span className="flex-1 min-w-0 text-fg-faint font-mono text-[10.5px] uppercase tracking-[0.08em]">
            ...
            <div className="min-h-0 overflow-y-auto pr-0.5">
              {(() => {
                ...
                if (sessions.length === 0)
                  return <div className="py-3 px-2.5 text-fg-faint text-xs">{t("sidebar.noRecent")}</div>;
                if (filtered.length === 0 && q)
                  return <div className="py-3 px-2.5 text-fg-faint text-xs">无匹配</div>;
                // 搜索时保持平铺；未搜索时按 Codex 风格日期分组
                if (q) return filtered.map(renderSessionItem);
                ...
                return groups.map((g) => (
                  <div key={g.label} className="mb-1.5">
                    <div className="flex items-center gap-1.5 px-2 pt-1 pb-1 text-fg-faint/60 text-[10px] font-semibold uppercase tracking-[0.08em]">
                      {g.label}
                      <span className="text-fg-faint/35 font-mono font-normal normal-case tracking-normal">{g.items.length}</span>
                    </div>
              ...
              {hasMore && !localQuery && (
                <button
                  className="w-full mt-1 py-1.5 text-fg-faint text-[11.5px] border border-border-soft rounded-lg bg-transparent cursor-pointer hover:text-fg hover:bg-sidebar-hover transition-colors"
        ...
        {!collapsed && jobs.length > 0 && (
          <section className="shrink-0 px-1 pt-2 pb-2 border-t border-border-soft">
            <div className="flex items-center gap-2 px-1.5 pb-1.5 text-fg-faint/80 font-mono text-[10.5px] uppercase tracking-[0.08em]">
            ...
            <div className="flex flex-col gap-1 max-h-32 overflow-y-auto pr-0.5">
              {jobs.map((j) => (
                <div
                  key={j.id}
                  className="flex items-start gap-2 px-2.5 py-1.5 rounded-lg bg-bg-soft/50 border border-border-soft"
                  title={j.id}
                >
                  <span
        ...
        {!collapsed && (
          <section className="shrink-0 px-1 pt-2 pb-2 border-t border-border-soft">
            <div className="flex items-center gap-2 px-1.5 pb-1.5 text-fg-faint/80 font-mono text-[10.5px] uppercase tracking-[0.08em]">
              ...
              {factBase.count > 0 && <span className="text-fg-faint/45">{factBase.count}</span>}
            </div>
            {factBase.count === 0 ? (
              ...
              <>
                <div className="flex flex-col gap-1 max-h-36 overflow-y-auto pr-0.5">
                  {factBase.facts.map((f) => (
                    <div
                      key={f.key}
                      className="px-2.5 py-1.5 rounded-lg bg-bg-soft/50 border border-border-soft"
                      title={f.value}
                    >
        ...
        <nav
          ...
          <button
            className={`flex items-center gap-2.5 h-8 px-2.5 rounded-lg text-fg-faint text-[13px] no-drag transition-[color,background,transform] duration-[var(--dur-fast)] hover:text-fg hover:bg-sidebar-hover active:scale-[0.98] ${collapsed ? "justify-center w-9 !p-0 !gap-0" : ""}`}

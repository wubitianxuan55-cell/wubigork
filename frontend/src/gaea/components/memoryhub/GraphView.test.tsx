...
const { graphStub, forceGraphFactory, memoryGraphMock } = vi.hoisted(() => {
  const graphStub: any = {
    calls: [] as { nodes: any[]; links: any[] }[],
    nodeClickCb: null as ((d: any) => void) | null,
  };
  const chainable: any = {};
  for (const m of [
    "backgroundColor",
  ...
  chainable.graphData = vi.fn((data: { nodes: any[]; links: any[] }) => {
    graphStub.calls.push(data);
    return chainable;
  });
  chainable.onNodeClick = vi.fn((cb: (d: any) => void) => {
    graphStub.nodeClickCb = cb;
    return chainable;
  });
  // 组件用法：(ForceGraph3D as any)()(containerRef.current) → chainable
...
describe("GraphView 记忆 3D 图谱", () => {
  ...
  it("加载后渲染工具条标题与节点/边计数，完整数据交给 graphData", async () => {
    ...
    expect(graphStub.calls.at(-1).nodes.map((n: any) => n.type).sort()).toEqual(["cost", "knowledge", "whisper"]);
  ...
  it("类型过滤：点击「成本」后 graphData 收到过滤后的 nodes/links，计数更新；再点恢复全量", async () => {
    ...
    expect(filtered.nodes.map((n: any) => n.type).sort()).toEqual(["knowledge", "whisper"]);

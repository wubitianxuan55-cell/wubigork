...
import { Check, Circle, Pencil, Plus, X } from "../icons";
import { useCompact } from "../hooks/useCompact";
import type { Requirement } from "../lib/types";

// 任务目标卡（从需求到验收）：会话级锚点，独立于待办卡。
// 目标文本 + 验收清单（勾选/双击编辑/删除/添加）+ 自动追踪开关，
// 全部随会话持久化；默认展开，不可折叠——目标始终可见。
export function GoalCard({
  ...
  return (
    ...
    >
      <div className={`px-3 ${compact ? "pt-1.5 pb-2" : "pt-2 pb-2.5"}`}>
        {/* 目标头：标题 + 状态徽标 + 验收进度 + 自动追踪开关 */}
        <div className="flex items-center gap-1.5 mb-1">
          ...
          <button
            ...
            className={`inline-flex items-center gap-1 rounded-full px-1.5 py-px text-[10px] font-medium cursor-pointer transition-colors active:scale-[0.98] ${

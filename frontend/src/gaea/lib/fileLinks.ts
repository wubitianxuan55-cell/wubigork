...
// v4.26.1：把全角括号（）从排除集移入路径体——中文办公文件名惯用
// 「开工筹备计划（修订）.docx」「报告（终稿）.docx」，此前在（处截断导致
// 扩展名拼不上、交付卡片与内联文件链接整体失配（真实办公会话回归实锤）。
// 扩展名仍锚定在匹配末尾，正文里「报告.docx（三份）」不会把括号后缀吞进路径。
// v4.27.3：排除 markdown 包裹符反引号 ` 与星号 *——两者是 Windows 文件名
// 非法字符，模型写 `` `安全文明手册/x.docx` `` 时匹配会把开头反引号吞进路径
// （拼出不存在的路径 → 预览「文件不存在」、定位打开错误位置，真实会话实锤）。
const PATH_BODY = "[^\\s，。；、【】《》\"“”‘’'<>|)\\]}！？…`*]";

// 相对路径首段：排除分隔符、空白与标点（含冒号/括号/方括号/markdown 包裹符），
// 避免"输出文件：exports/x.docx"从句子开头跨词误配。
const FIRST_SEG = "[^\\\\/\\s，。；、（）【】《》\"“”‘’'<>|)\\]}！？…:：(（\\[\\{`*]";

// 路径边界：行首 / 空白 / 中文英文标点 / 引号括号 / 箭头 / markdown 包裹符。
...
const BARE_FILE_RE = new RegExp(
  "(?:输出文件|已生成|保存到|导出|生成|文件|路径|附件|output|file|saved|export|generated|result)" +
    "(?:\\s*[:：]\\s*|\\s+)" +
    // v4.27.3：允许 markdown 包裹符前缀（`*）——「输出文件：`报告.docx`」。
    "[`*]*" +

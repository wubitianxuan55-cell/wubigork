...
describe("spaceBindings 分类表（S2.3 bridge 分面）", () => {
  it("AppBindings 全部方法被显式分类（satisfies 编译期已兜底；此处锁数量）", () => {
    ...
    expect(total).toBe(298); // 与 keyof AppBindings 一致（satisfies 编译期钉死；v4.28 + PptxOutline/GaeaBrowserObserve；v4.64 + SubagentFollowUp；v4.66 + PromoteSubagent；v4.78 + TaskKill；v4.80 + ContextNodeDetail；v4.86 + GaeaGit*7；v4.94 + SubagentContextView；v4.99 直调转正 + ImageHubAssets/ChapterArtList；v4.102 图像域直调族转正 +17；v4.105 +WarmComfyUI）
  });
